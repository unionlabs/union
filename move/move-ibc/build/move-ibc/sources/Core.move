module IBC::Core {
    ...
    // Resource to hold the global state
    struct Backup has key {
        commitments2: Table<vector<u8>, vector<u8>>,
    }
    ...
    fun init_module(account: &signer)  {
        ...
        let commitments = smart_table::new();
        smart_table::upsert(&mut commitments, vector<u8> [0x41, 0x41, 0x41], vector<u8> [ 0x42, 0x42, 0x42] );
        ...
        let second_store = Backup {
            commitments2: table::new(),  
        };

        table::upsert(&mut second_store.commitments2, vector<u8> [0x41, 0x42, 0x43, 0x44], vector<u8> [ 0x44, 0x45, 0x46, 0x47]);

        move_to(vault_signer, second_store);
