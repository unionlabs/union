module IBC::connection_end {
    ...
    public fun new(
        ...
        ConnectionEnd { client_id, versions, state, delay_period, counterparty }
    }
    ...
    public fun new_counterparty(
        ...
        Counterparty {
            client_id,
            connection_id,
            prefix: MerklePrefix { key_prefix: prefix }
        }
    }
    ...
    public fun new_versions(
        identifiers: vector<String>, features: vector<vector<String>>
    ): vector<Version> {
        let i = 0;
    ...
    public fun set_versions(
        connection_end: &mut ConnectionEnd, versions: vector<Version>
    ) {
    ...
    public fun conn_counterparty_key_prefix(
        connection_end: &ConnectionEnd
    ): &vector<u8> {
    ...
    public fun set_version_features(
        version: &mut Version, features: vector<String>
    ) {
        version.features = features;
    }

    public fun set_version_identifier(
        version: &mut Version, identifier: String
    ) {
    ...
    fun decode_merkle_prefix(
        buf: &vector<u8>,
        cursor: u64,
        len: u64,
        prefix: &mut MerklePrefix
    ): (u64, u64) {
        let first_pos = cursor;
        while (cursor - first_pos < len) {
            let (tag, wire_type, advance, err) = proto_utils::decode_prefix(buf, cursor);
        ...
        while (cursor - first_pos < len) {
            ...
            let advance =
                if (tag == 1) {
                    ...
                    cursor = cursor + advance;
                    let (n_read, err) =
                        decode_merkle_prefix(buf, cursor, len, &mut counterparty.prefix);
                    if (err != 0 || n_read != len) {
                        return (0, err)
                    };
                    len
                ...
                if (tag == 1) {
                    ...
                    let (n_read, err) =
                        decode_counterparty(
                            ...
                            cursor,
                            len,
                            &mut connection_end.counterparty
                        );
                    if (err != 0 || n_read != len) {
                        return option::none()
                    };
    ...
    public fun encode_proto(end: ConnectionEnd): vector<u8> {
        ...
        if (!vector::is_empty(&end.versions)) {
            ...
            while (i < vector::length(&end.versions)) {
                ...
                vector::append(
                    &mut buf, proto_utils::encode_varint(vector::length(&version))
                );
    ...
    fun encode_proto_version(value: Version): vector<u8> {
        ...
        while (i < vector::length(&value.features)) {
            vector::append(
                &mut buf,
                proto_utils::encode_string(2, *vector::borrow(&value.features, i))
            );
            i = i + 1;
        };
        buf
    ...
    fun encode_proto_counterparty(value: Counterparty): vector<u8> {
        ...
        if (!vector::is_empty(&merkle_prefix)) {
            ...
            vector::append(
                &mut buf, proto_utils::encode_varint(vector::length(&merkle_prefix))
            );
