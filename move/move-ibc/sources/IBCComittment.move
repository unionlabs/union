module IBC::IBCCommitment {
    ...
    public fun keccak256(s: String): vector<u8> {
        let vec_val = *StringModule::bytes(&s);
        hash::sha2_256(vec_val)
    }

    // Generate the path for client state
    public fun client_state_path(client_id: String): String {
        let path = StringModule::utf8(b"clients/");
        StringModule::append(&mut path, client_id);
        StringModule::append_utf8(&mut path, b"/clientState");
        path
    }
    ...
    public fun consensus_state_path(client_id: String, revision_number: u64, revision_height: u64): String {
        let path = StringModule::utf8(b"clients/");
        StringModule::append(&mut path, client_id);
        StringModule::append_utf8(&mut path, b"/consensusStates/");
        StringModule::append(&mut path, string_utils::to_string(&revision_number));
        StringModule::append_utf8(&mut path, b"-");
        StringModule::append(&mut path, string_utils::to_string(&revision_height));
        path
    }

    // Generate the path for connection
    public fun connection_path(connection_id: String): String {
        let path = StringModule::utf8(b"connections/");
        StringModule::append(&mut path, connection_id);
        path
    }

    // Generate the path for channel
    public fun channel_path(port_id: String, channel_id: String): String {
        let path = StringModule::utf8(b"channelEnds/ports/");
        StringModule::append(&mut path, port_id);
        StringModule::append_utf8(&mut path, b"/channels/");
    ...
    public fun packet_commitment_path(port_id: String, channel_id: String, sequence: u64): String {
        let path = StringModule::utf8(b"commitments/ports/");
        StringModule::append(&mut path, port_id);
        StringModule::append_utf8(&mut path, b"/channels/");
        StringModule::append(&mut path, channel_id);
        StringModule::append_utf8(&mut path, b"/sequences/");
        StringModule::append(&mut path, string_utils::to_string(&sequence));
        path
    }
    ...
    public fun packet_acknowledgement_commitment_path(port_id: String, channel_id: String, sequence: u64): String {
        let path = StringModule::utf8(b"acks/ports/");
        StringModule::append(&mut path, port_id);
        StringModule::append_utf8(&mut path, b"/channels/");
        StringModule::append(&mut path, channel_id);
        StringModule::append_utf8(&mut path, b"/sequences/");
    ...
    public fun packet_receipt_commitment_path(port_id: String, channel_id: String, sequence: u64): String {
        let path = StringModule::utf8(b"receipts/ports/");
        StringModule::append(&mut path, port_id);
        StringModule::append_utf8(&mut path, b"/channels/");
        StringModule::append(&mut path, channel_id);
        StringModule::append_utf8(&mut path, b"/sequences/");
    ...
    public fun next_sequence_send_commitment_path(port_id: String, channel_id: String): String {
        let path = StringModule::utf8(b"nextSequenceSend/ports/");
        StringModule::append(&mut path, port_id);
        StringModule::append_utf8(&mut path, b"/channels/");
        StringModule::append(&mut path, channel_id);
        path
    ...
    public fun next_sequence_recv_commitment_path(port_id: String, channel_id: String): String {
        let path = StringModule::utf8(b"nextSequenceRecv/ports/");
        StringModule::append(&mut path, port_id);
        StringModule::append_utf8(&mut path, b"/channels/");
        StringModule::append(&mut path, channel_id);
        path
    ...
    public fun next_sequence_ack_commitment_path(port_id: String, channel_id: String): String {
        let path = StringModule::utf8(b"nextSequenceAck/ports/");
        StringModule::append(&mut path, port_id);
        StringModule::append_utf8(&mut path, b"/channels/");
        StringModule::append(&mut path, channel_id);
        path
