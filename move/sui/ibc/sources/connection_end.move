module ibc::connection_end {
    ...
    public struct Version has copy, store, drop {
        identifier: String,
    ...
    public struct Counterparty has copy, store, drop {
        client_id: String,
        connection_id: String,
        prefix: MerklePrefix,
    }

    public struct MerklePrefix has copy, store, drop {
    ...
    public fun new_versions(
        ...
        while (i < identifiers.length()) {
            ...
            let f = features[i];
            versions.push_back(new_version(ident, f));
            i = i + 1;
        };
        versions
    }
    ...
    fun decode_version(buf: &vector<u8>, mut cursor: u64, len: u64, version: &mut Version): (u64, u64) {
        ...
        while (cursor - first_pos < len) {
            ...
            let advance = if (tag == 1) {
                let (mut str, advance) = proto_utils::decode_string(wire_type, buf, cursor);
                if (option::is_none(&str)) {
                    return (0, 1)
                };
                version.identifier = str.extract();
                advance
            } else if (tag == 2) {
                let (mut str, advance) = proto_utils::decode_string(wire_type, buf, cursor);
                if (str.is_none()) {
                ...
                version.features.push_back(str.extract());
                advance
            } else {
                return (0, 1)
            };
    ...
    fun decode_merkle_prefix(buf: &vector<u8>, mut cursor: u64, len: u64, prefix: &mut MerklePrefix): (u64, u64) {
        let first_pos = cursor;
        while (cursor - first_pos < len) {
            let (tag, wire_type, advance, err) = proto_utils::decode_prefix(buf, cursor);
            if (err != 0) {
            ...
            cursor = cursor + advance;
            let advance = if (tag == 1) {
                let (mut bytes, advance) = proto_utils::decode_bytes(wire_type, buf, cursor);
                if (bytes.is_none()) {
                    return (0, 1)
                };
                prefix.key_prefix = bytes.extract();
                advance
            } else {
                return (0, 1)
            };
    ...
    fun decode_counterparty(buf: &vector<u8>, mut cursor: u64, len: u64, counterparty: &mut Counterparty): (u64, u64) {
        let first_pos = cursor;
        while (cursor - first_pos < len) {
            let (tag, wire_type, advance, err) = proto_utils::decode_prefix(buf, cursor);
            if (err != 0) {
            ...
            let advance = if (tag == 1) {
                ...
                counterparty.client_id = str.extract();
                advance
            } else if (tag == 2) {
                let (mut str, advance) = proto_utils::decode_string(wire_type, buf, cursor);
                if (option::is_none(&str)) {
                ...
                counterparty.connection_id = str.extract();
                advance
    ...
    public fun decode_proto(buf: vector<u8>): Option<ConnectionEnd> {
        ...
        let mut cursor = 0;
        let mut connection_end = default();
        while (cursor < vector::length(&buf)) {
            ...
            let n_read = if (tag == 1) {
                ...
                connection_end.client_id = str.extract();
                advance
            } else if (tag == 2) {                
                let (len, advance, err) = proto_utils::decode_nested_len(wire_type, &buf, cursor);
                if (err != 0) {
                ...
                connection_end.versions.push_back(version);
                len
            } else if (tag == 3) {
    ...
    public fun encode_proto(end: ConnectionEnd): vector<u8> {
        ...
        if (!end.client_id.is_empty()) {
            buf.append(proto_utils::encode_string(1, end.client_id));
        };

        if (!end.versions.is_empty()) {
            let mut i = 0;
            while (i < end.versions.length()) {
                let version = encode_proto_version(end.versions[i]);
                buf.append(proto_utils::encode_prefix(2, 2));
                buf.append(proto_utils::encode_varint(version.length()));
                buf.append(version);
                i = i + 1;
            };
        };
        ...
        if (end.state != 0) {
            buf.append(proto_utils::encode_u64(3, end.state));
        };

        let counterparty = encode_proto_counterparty(end.counterparty);
        if (!counterparty.is_empty()) {
            buf.append(proto_utils::encode_prefix(4, 2));
            buf.append(proto_utils::encode_varint(counterparty.length()));
            buf.append(counterparty);
        };

        if (end.delay_period != 0) {
            buf.append(proto_utils::encode_u64(5, end.delay_period));
        };
    ...
    fun encode_proto_version(value: Version): vector<u8> {
        let mut buf = proto_utils::encode_string(1, value.identifier);
        let mut i = 0;
