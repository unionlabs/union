module ibc::proto_utils {
    ...
    public fun encode_string(field: u8, value: String): vector<u8> {
        let mut prefix = encode_varint(((((field << 3) as u8) | 2) as u64));
    ...
    public fun decode_untagged_bytes(buf: &vector<u8>, mut cursor: u64): (Option<vector<u8>>, u64) {
        let (bytes_len, advance, err) = decode_varint_raw(buf, cursor);
        cursor = cursor + advance;
        if (err != 0) {
            return (option::none(), 0)
        };
        // TODO(aeryz): figure out how to do slice
        (option::none(), 0)
        // (option::some(vector::slice(buf, cursor, cursor + bytes_len)), advance + bytes_len)
    }

    public fun decode_untagged_string(buf: &vector<u8>, mut cursor: u64): (Option<String>, u64) {
    ...
    fun decode_varint_raw_slice(buf: &vector<u8>, cursor: u64): (u64, u64, u64) {
        ...
        let mut part1 = part1 + ((b as u32) << 7);
        if (b < 0x80) {
        ...
        let mut part1 = part1 + ((b as u32) << 14);
        if (b < 0x80) {
        ...
        let mut part1 = part1 + ((b as u32) << 21);
        if (b < 0x80) {
    ...
    fun decode_varint_raw_slow(buf: &vector<u8>, cursor: u64): (u64, u64, u64) {
        let mut value = 0u64;
        let mut rem = vector::length(buf) - cursor;
        if (rem > 10) {
            rem = 10;
        };
    ...
    public fun encode_u64(field: u8, value: u64): vector<u8> {
        let mut prefix = encode_prefix(field, 0);
