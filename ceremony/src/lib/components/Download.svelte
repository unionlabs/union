<script lang="ts">
  ...
  $effect(() => {
    if (!generated) {
      ...
      terminal.updateHistory("The MPC client automatically uses this secret to sign your contribution.")
      terminal.updateHistory("Your secret is locally generated through the MPC client.")
    } else {
      terminal.updateHistory("Please store your secret somewhere safe, such as in your password manager. There's no need to open the file and remember to never share a secret. This secret key is the only way to prove that you have contributed.")
    }
  })
