...
>
  ...
  <div class="w-full flex justify-between bg-neutral-800/80 px-4 py-2">
    <div class="flex items-center gap-2">
      ...
      >
        ...
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M40 80H8V112H40V80ZM136 80H104V112H136V80Z"
          fill="#fff"
        />
        <path
          fill-rule="evenodd"
          clip-rule="evenodd"
          d="M152 112L184 112L184 80L152 80L152 112ZM56 112L88 112L88 80L56 80L56 112Z"
          fill="#fff"
        />
        <path
          fill-rule="evenodd"
      ...
      <Button
        onclick={() => changeTab(1)}
        class={cn(terminal.tab === 1 ? "text-union-accent-500" : "text-white")}
      >CORE
      </Button>
      <Button
        onclick={() => changeTab(2)}
        class={cn(terminal.tab === 2 ? "text-union-accent-500" : "text-white")}
      >LOGS
      </Button>
      <Button
        onclick={() => changeTab(3)}
        class={cn(terminal.tab === 3 ? "text-union-accent-500" : "text-white")}
      >USERS
      </Button>
      {#if terminal.hash !== undefined}
        <Button
          onclick={() => changeTab(4)}
          class={cn(terminal.tab === 4 ? "text-union-accent-500" : "text-white")}
        >
          {#if terminal.hash}
    ...
    {#if user.session?.user}
      <Button
        class="text-white"
        onclick={() => logout(terminal, contributor)}
      >Log out</Button>
    {/if}
  </div>
  ...
  <!--TERMINAL-->
  <div
    class="overflow-y-scroll h-full p-4"
    use:autoScroll
  >
