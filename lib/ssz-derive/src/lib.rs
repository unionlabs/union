...
//! use ssz::types::{typenum::U4, VariableList};
//!
...
use quote::quote;
use syn::{parse_macro_input, Attribute, DataEnum, DataStruct, DeriveInput, Ident, Index, Meta};
...
struct TreeHashStructOpts {
    #[darling(default)]
    enum_behaviour: Option<String>,
}

const ENUM_VARIANTS: &[&str] = &[ENUM_TRANSPARENT, ENUM_UNION];
const TREE_HASH_NO_ENUM_BEHAVIOUR_ERROR: &str = "enums require an \"enum_behaviour\" attribute, \
    e.g., #[tree_hash(enum_behaviour = \"transparent\")]";

enum TreeHashEnumBehaviour {
    Transparent,
    Union,
}

impl TreeHashEnumBehaviour {
    pub fn new(s: Option<String>) -> Option<Self> {
        s.map(|s| match s.as_ref() {
            ENUM_TRANSPARENT => TreeHashEnumBehaviour::Transparent,
            ENUM_UNION => TreeHashEnumBehaviour::Union,
            other => panic!(
...
pub fn tree_hash_derive(input: TokenStream) -> TokenStream {
    let item = parse_macro_input!(input as DeriveInput);
    let opts = TreeHashStructOpts::from_derive_input(&item).unwrap();
    let enum_opt = TreeHashEnumBehaviour::new(opts.enum_behaviour);
    ...
    match &item.data {
        ...
        syn::Data::Enum(s) => match enum_opt.expect(TREE_HASH_NO_ENUM_BEHAVIOUR_ERROR) {
            TreeHashEnumBehaviour::Transparent => tree_hash_derive_enum_transparent(&item, s),
            TreeHashEnumBehaviour::Union => tree_hash_derive_enum_union(&item, s),
        },
...
fn tree_hash_derive_struct(item: &DeriveInput, struct_data: &DataStruct) -> TokenStream {
    ...
    let output = quote! {
        impl #impl_generics ::ssz::tree_hash::TreeHash for #name #ty_generics #where_clause {
            fn tree_hash_type() -> ::ssz::tree_hash::TreeHashType {
                ::ssz::tree_hash::TreeHashType::Container
            }
...
fn tree_hash_derive_enum_transparent(
    ...
    let (patterns, type_exprs): (Vec<_>, Vec<_>) = enum_data
        ...
        .map(|variant| {
            ...
            let ty = &(&variant.fields).into_iter().next().unwrap().ty;
            let type_expr = quote! {
                <#ty as ::ssz::tree_hash::TreeHash>::tree_hash_type()
            };
            (pattern, type_expr)
        })
        .unzip();
