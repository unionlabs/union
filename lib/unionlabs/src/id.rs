...
pub struct ClientId(#[doc(hidden)] u32);
...
impl ClientId {
    ...
    pub fn parse_prefixed(s: &str) -> Result<(&str, Self), ParsePrefixedIdError> {
        ...
        id.parse()
            .map(|id| (prefix, Self(id)))
            .map_err(ParsePrefixedIdError::ParseIntError)
    }

    #[must_use]
    pub fn to_string_prefixed(&self, prefix: &str) -> String {
...
impl ConnectionId {
    ...
    pub fn parse_prefixed(s: &str) -> Result<Self, ParsePrefixedIdError> {
...
impl ChannelId {
    ...
    #[must_use]
    pub fn to_string_prefixed(&self) -> String {
        format!("{CHANNEL_ID_PREFIX}-{}", self.0)
    }
