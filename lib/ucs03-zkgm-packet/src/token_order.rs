...
pub enum TokenOrderV2Metadata {
    ...
    Escrow { metadata: Bytes },
    Unescrow { metadata: Bytes },
...
impl TokenOrderV2Metadata {
    ...
    pub(crate) fn encode(self) -> (u8, Bytes) {
        match self {
            ...
            TokenOrderV2Metadata::Escrow { metadata: bytes } => (TOKEN_ORDER_KIND_ESCROW, bytes),
            TokenOrderV2Metadata::Unescrow { metadata: bytes } => {
                (TOKEN_ORDER_KIND_UNESCROW, bytes)
            }
