...
use crate::{
    cosmos_sdk::CosmosSdkChain, private_key::PrivateKey, union::UnionEventSourceError, ChainEvent,
    EventSource, Pool,
};
...
impl EventSource for Cosmos {
    type Event = ChainEvent<Self>;
    // its the same for now leave me alone
                ...
                move |(this, previous_height)| async move {
                        ...
                        (previous_height.revision_height + 1)..=(current_height.revision_height)
                    {
                        let response = if let Ok(res) = this
                            .tm_client
                            .tx_search(tendermint_rpc::query::Query::eq("tx.height", h), false, 1, 255, tendermint_rpc::Order::Descending)
                            .await
                        {
                            res
                        } else {
