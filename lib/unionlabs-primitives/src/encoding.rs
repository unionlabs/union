...
pub trait Encoding: Sealed {
    ...
    /// # Errors
    ///
    /// This method should only fail if writing to `f` fails. See the documentation on [`core::fmt::Display::fmt`] for more information.
...
impl From<hex::FromHexError> for HexPrefixedFromStrError {
...
impl Encoding for HexUnprefixed {
    ...
    fn decode_to_slice<T: AsRef<[u8]>>(data: T, out: &mut [u8]) -> Result<(), Self::Error> {
        hex::decode_to_slice(data, out).map_err(Into::into)
    }
}
...
impl From<hex::FromHexError> for HexUnprefixedFromStrError {
    fn from(v: hex::FromHexError) -> Self {
        Self::InvalidHex(v)
    }
}
