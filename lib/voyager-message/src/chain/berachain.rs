...
use crate::{
    ...
    id, AnyChainIdentified, ChainExt, DoAggregate, Identified, IsAggregateData, VoyagerMessage,
};
...
{
    fn fetch_block_range(c: &Berachain, range: FetchBlockRange<Berachain>) -> Op<VoyagerMessage> {
        fetch(id(
            c.chain_id(),
...
where
    ...
    AnyChainIdentified<AnyFetch>: From<Identified<Berachain, Fetch<Berachain>>>,
{
    async fn do_fetch(c: &Berachain, msg: Self) -> Op<VoyagerMessage> {
        match msg {
