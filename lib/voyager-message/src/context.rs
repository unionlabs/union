...
pub struct Modules {
    ...
    client_modules:
        HashMap<(ClientType<'static>, IbcInterface<'static>, IbcVersion), ModuleRpcClient>,
            ...
            |ClientModuleInfo {
             ...
             rpc_client| {
                let prev = modules.client_modules.insert(
                    (client_type.clone(), ibc_interface.clone(), *ibc_version),
                    rpc_client,
                );
...
impl Modules {
    pub fn info(&self) -> LoadedModulesInfo {
        ...
        let client = self
            ...
            .iter()
            .map(
                |((client_type, ibc_interface, ibc_version), _)| ClientModuleInfo {
    ...
    pub fn client_module<'a, 'b, 'c: 'a>(
        ...
        match self
            .client_modules
            .get(&(client_type.clone(), ibc_interface.clone(), ibc_version))
        {
            Some(client_module) => Ok(client_module.client()),
            None => Err(ClientModuleNotFound::ModuleNotFound {
                client_type: client_type.clone().into_owned(),
...
pub enum ClientModuleNotFound {
    ...
    #[error(
        "no module loaded for client type `{client_type}`, IBC \
        interface `{ibc_interface}`, and IBC version `{ibc_version}`"
    )]
    ModuleNotFound {
