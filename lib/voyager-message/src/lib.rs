...
mod tests {
    ...
    use voyager_core::{ChainId, ClientType, IbcInterface};
    use voyager_vm::{call, data, Op};

    use super::*;
    use crate::{call::MakeMsgCreateClient, data::IbcMessage};

    #[test]
    fn handshake_messages() {
        print_json(call(MakeMsgCreateClient {
            chain_id: ChainId::new("union-devnet-1"),
            height: QueryHeight::Latest,
            metadata: Value::Null,
            counterparty_chain_id: ChainId::new("stargaze-devnet-1"),
            ibc_interface: IbcInterface::new(IbcInterface::IBC_GO_V8_08_WASM),
            client_type: ClientType::new(ClientType::TENDERMINT),
        }));

        print_json(call(MakeMsgCreateClient {
            chain_id: ChainId::new("stargaze-devnet-1"),
            height: QueryHeight::Latest,
            metadata: Value::Null,
            counterparty_chain_id: ChainId::new("union-devnet-1"),
            ibc_interface: IbcInterface::new(IbcInterface::IBC_GO_V8_08_WASM),
