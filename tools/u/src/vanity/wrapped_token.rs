...
use clap::Args;
use rand::RngCore;
use unionlabs::ethereum::slot::keccak256;
...
impl Cmd {
    pub async fn run(self) -> anyhow::Result<()> {
        let token_bytes = self.token.into_vec();
        let auth_address: Address = self.auth.get().into();
        let zkgm_address: Address = self.zkgm.get().into();
        let impl_address: Address = self.impl_address.get().into();
        ...
        println!("Channel ID: {}", self.channel_id);
        ...
        for _ in 0..self.threads {
            ...
            let handle = thread::spawn(move || -> Option<(Vec<u8>, Address)> {
                ...
                while !found.load(Ordering::Relaxed) {
                    ...
                    {
                        rng.fill_bytes(
                            &mut preimage[preimage_salt_index..preimage_salt_index + 20],
                        );
                    }
                    {
                        ...
                        let wrapped_token = create3::predict_deterministic_address(
                            zkgm_address,
                            wrapped_token_salt,
                        );
