...
// use base_client::{finalized_execution_block_of_l1_height, output_index_of_l2_block_on_l1_block};
use base_light_client_types::{
    header::{L2Header, OutputRootProof},
    ClientState, Header,
};
// use base_verifier::FINALIZATION_PERIOD_SECONDS;
...
impl Module {
    ...
    pub async fn fetch_dispute_game_factory_proxy_proof(
        &self,
        block_number: u64,
        ...
        let account_update = self
            ...
            .map_err(|e| {
                ErrorObject::owned(
                    -1,
                    ErrorReporter(e)
                        .with_message("error fetching dispute game factory proxy proof"),
                    None::<()>,
                )
            })?;

        debug!(storage_hash = %account_update.storage_hash, "fetched dispute game factory proxy proof");
...
impl Module {
    ...
    async fn fetch_l2_update(
        ...
        // the L1.
        let output_proposal_proof = self.fetch_game_proof(output_index, l1_height).await?;
        let l2_oracle_account_proof = self
            .fetch_dispute_game_factory_proxy_proof(l1_height)
            .await?;

        // Extract proofs for the preimage of the output proposal committed on
        // L1 (extracted above) and IBC handle contract on the L2.
        let output_root_proof = self
            .fetch_output_root_proof(l2_block_number.header.number)
            .await?;
        let l2_ibc_account_proof = self
            .fetch_ibc_contract_root_proof(l2_block_number.header.number)
            .await?;

        Ok(data(OrderedHeaders {
            headers: vec![(
                DecodedHeaderMeta {
                    height: Height::new(l2_block_number.header.number),
                },
                into_value(Header {
                    l1_height,
                    l2_ibc_account_proof,
                    l2_header: L2Header {
                        parent_hash: l2_block_number.header.parent_hash.into(),
                        sha3_uncles: l2_block_number.header.ommers_hash.into(),
                        miner: l2_block_number.header.beneficiary.into(),
                        state_root: l2_block_number.header.state_root.into(),
                        transactions_root: l2_block_number.header.transactions_root.into(),
                        receipts_root: l2_block_number.header.receipts_root.into(),
                        logs_bloom: Box::new(l2_block_number.header.logs_bloom.0.into()),
                        difficulty: l2_block_number.header.difficulty.into(),
                        number: l2_block_number.header.number.into(),
                        gas_limit: l2_block_number.header.gas_limit,
                        gas_used: l2_block_number.header.gas_used,
                        timestamp: l2_block_number.header.timestamp,
                        extra_data: l2_block_number
                            .header
                            .extra_data
                            .to_vec()
                            .try_into()
                            .unwrap(),
                        mix_hash: l2_block_number.header.mix_hash.unwrap_or_default().into(),
                        nonce: l2_block_number.header.nonce.unwrap_or_default().into(),
                        base_fee_per_gas: l2_block_number
                            .header
                            .base_fee_per_gas
                            .unwrap_or_default()
                            .into(),
                        withdrawals_root: l2_block_number.header.withdrawals_root.unwrap().into(),
                        blob_gas_used: l2_block_number.header.blob_gas_used.unwrap(),
                        excess_blob_gas: l2_block_number.header.excess_blob_gas.unwrap(),
                        parent_beacon_block_root: l2_block_number
                            .header
                            .parent_beacon_block_root
                            .unwrap()
                            .into(),
                        requests_hash: l2_block_number.header.requests_hash.map(Into::into).into(),
                    },
                    output_root_proof,
                    dispute_game_factory_account_proof: todo!(),
                    game_index: todo!(),
                    game_proof: todo!(),
                    game_account_proof: todo!(),
                    game_account_code: todo!(),
                }),
            )],
        }))
    }
