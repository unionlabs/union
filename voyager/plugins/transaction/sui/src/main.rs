...
use ucs03_zkgm::com::{TokenMetadata, TokenOrderV2, ZkgmPacket, TOKEN_ORDER_KIND_INITIALIZE};
use unionlabs::{
...
async fn register_token_if_zkgm(
    ...
    let Ok(fao) = TokenOrderV2::abi_decode_params(&zkgm_packet.instruction.operand) else {
        return Ok(None);
    };
