...
impl ModuleContext for Module {
    ...
    async fn new(config: Self::Config) -> Result<Self, BoxDynError> {
        ...
        let raw_chain_id = provider.get_chainid().await?;
        let chain_id = ChainId::new(U256(raw_chain_id).to_string());
        ...
        Ok(Self {
            ...
            keyring: ConcurrentKeyring::new(
                ...
                config.keyring.keys.into_iter().map(|config| {
                    ...
                    let wallet =
                        LocalWallet::new_with_signer(signing_key, address, raw_chain_id.as_u64());
