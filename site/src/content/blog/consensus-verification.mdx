...
import merkleTree2 from "#/assets/images/consensus-verification-blog/merkle-tree-1.svg";
...
*Union democratizes fundamental web3 ethos' desired mechanism  of cross-ecosystem connection.*
...
Users and liquidity are becoming increasingly fragmented across a growing number of L1 and L2 execution layers in Web3. A secure means of interoperability to enable communication among these isolated systems is critical. 
...
For users to seamlessly interact across ecosystems and to unify liquidity across different networks, without losing composability among applications, it's essential for blockchains to exchange and verify data accurately. Trust-minimized consensus verification, built by Union, addresses this challenge by verifying one chain's state in the execution environment of another, without introducing additional risk or trust assumptions.

## A Standard for Trust-Minimized Cross-Chain Consensus Verification

*IBC and its Limitations*

IBC exemplifies state verification across isolated blockchains, minimizing the need for trust in external entities. [IBC](https://arxiv.org/pdf/2006.15918.pdf), designed as a consensus-agnostic, cross-chain messaging protocol for heterogeneous blockchains, aims to be the standard for trust-minimized cross-chain consensus verification. Rooted in the Cosmos ecosystem, IBC serves as the native interoperability solution for Cosmos SDK-based chains and has garnered widespread adoption and continues to grow at an impressive rate.

<img
  alt='Packet Passing'
  src={packetPassing.src}
/>

To understand how IBC works, you have to be aware of some basic principles of how blockchains handle state data. Ideally, you would want to store everything on-chain, secured on thousands of computers across the globe with extremely high availability and integrity guarantees.

Unfortunately, it is often not feasible due to the scarcity of block space as a resource on chains like Ethereum, where storing a 32-byte word typically already costs 20,000 gas (approximately $3 at the time of writing). Storage space is limited and expensive. To solve this problem the Ethereum ecosystem developed many alternative ways to store data in a decentralized fashion. While they usually involve a tradeoff between availability and price, data integrity is ensured through the Merkle tree structure blockchains use to store data.

More specifically, blockchains like Ethereum use a tree data structure (Merkle Tree) where each node is a hash of the nodes below it, to repeatedly hash different subsets of the data. This enables you to just send a hash instead of the full data you are looking to prove inclusion/exclusion of by using [Merkle inclusion proofs](https://ethereum.org/developers/tutorials/merkle-proofs-for-offline-data-integrity).
...
This is exactly what IBC is doing. IBC-enabled chains pass block headers in IBC Packets through IBC channels to other networks, where an IBC light client verifies the validator signatures. This allows these IBC-enabled chains to effectively prove any kind of statement about the source chain’s state, unlocking powerful cross-chain composability.

## The Status Quo of the Interoperability Landscape
*Trusted Solutions getting rekt*

While IBC is a very promising solution to overcome the existing friction between different blockchains, it is not always feasible to verify validator signatures directly within a blockchain’s execution environment, due to computational limitations. One such example is Ethereum, where an IBC implementation would typically not be feasible within the gas limits of EVM execution.

This has led to the emergence of a myriad of approaches aiming to enable scalable means of verifying a certain consensus having been reached, across heterogenous blockchains. Using off-chain committees performing multisig-based message verification, PoS validator sets or fraud proving mechanisms have proven to be popular choices. While these have been a feasible way to enable interoperability quickly, they also introduce significant additional trust assumptions outside of the security guarantees provided by the underlying blockchains. This can pose risks and requires the user to trust potentially centralized operators.
...
All of these before-mentioned solutions, further referred to as “trusted”, introduce some degree of centralization or human element. Consequently, attack vectors arise which have historically made these interoperability solutions, or more specifically the bridging protocols built on top of them, very prone to hacks or other forms of malicious value extraction. [This is impressively evidenced by many large-scale exploits in the past (including Ronin, Multichain, Horizon, and Wormhole)](https://rekt.news/leaderboard/).

## Introducing Union
*Trust-Minimized Layer for Cross-Chain Consensus Verification Powered by ZK*

Trust-minimized cross-chain consensus verification must become more efficient to overcome the existing security risks in trusted interoperability designs and establish a truly global, trust-minimized interoperability layer - exactly what Union’s interoperability solution is specialized to do. Union expands IBC beyond the borders of the Cosmos ecosystem and provides a global interoperability layer for block header based consensus verification.
