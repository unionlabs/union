...
If you do not already have one, you could follow [this guide](/docs/joining-testnet/getting-tokens).
...
uniond query ibc connection end $UNION_CONNECTION_ID --node https://rpc.testnet.bonlulu.uno:443 	
...
```bash
# Query the connection on Union
uniond query ibc channel end $UCS01_PORT_ID $UNION_CHANNEL_ID --node https://rpc.testnet.bonlulu.uno:443 	
