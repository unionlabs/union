...
Although most HTTP clients can be used to query our API, we recommend [Apollo](https://www.apollographql.com/docs/react/). Our graphql schema can
be downloaded using [graphqurl](https://github.com/hasura/graphqurl) or the [Apollo CLI](https://www.apollographql.com/docs/devtools/cli/):
...
apollo schema:download --endpoint https://graphql.union.build/v1/graphql
```
...
query GetLatest10Transfers @cached(ttl: 1) {
  v1_transfers(limit: 10, order_by: {source_timestamp: desc}) {
    assets
	  receiver
	  sender
  }
...
```
Aggregate statistics about all transfers and packets are also available, such as the total amount of transfers:

```graphql
query GetTransferCount @cached(ttl: 1) {
  v1_transfers_aggregate {
    aggregate {
      count
    }
  }
...
query GetLatest10Packets @cached(ttl: 1) {
  v1_packets(limit: 10, order_by: {source_time: desc}) {
    source_chain {
      display_name
    }
    destination_chain {
...
Both transfers and packets come with traces to track the progress of the message. There are 5 different trace types:
...
query GetTracesByHash @cached(ttl: 1) {
  v1_traces(where: {initiating_transaction_hash: {_eq: "91C934788FD13D5325F5A1D9CF1634D1861E318D6DDD2C141E91CBE8A68A053D"}}) {
    type
	  transaction_hash
    chain {
      display_name
...
The `v0_traces.data` field is dependent on the type and may contain chain-specific information as well. The `v1_transfers` and `v1_packets` 
