...
*Where `RPC_ENDPOINT` is an RPC endpoint from the [Public Endpoints](../public-endpoints) list.*
...
trust_period = "400s"
```
*Where `STAKE_SYNC_NODE_1` & `STAKE_SYNC_NODE_2` can be the same node. The node(s) must support state sync.*
