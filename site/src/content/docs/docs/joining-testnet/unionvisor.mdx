...
We release container images of Unionvisor called bundles. Each bundle contains everything required for running Unionvisor and joining a particular network. The Unionvisor bundle for <code><FunctionResult result={import.meta.env.CHAIN_ID_VERSION} /></code> is `bundle-testnet-7`. You can obtain the Unionvisor bundle for <code><FunctionResult result={import.meta.env.CHAIN_ID_VERSION} /></code> from our [GitHub Container Registry](https://github.com/orgs/unionlabs/packages/container/package/bundle-testnet-7).
