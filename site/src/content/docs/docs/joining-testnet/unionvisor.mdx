...
_Where `UNIONVISOR_VERSION` is <Version chainId="union-testnet-6" genesis={false} />_
...
After the `unionvisor init` command is done running, you should have a `.unionvisor` folder with the following contents:
...
- versions
  - <Version chainId="union-testnet-6" genesis={false} />
    - uniond
