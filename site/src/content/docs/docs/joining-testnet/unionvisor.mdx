...
sidebar:
  order: 2
---
import Version from "#/components/chain-versions.svelte"
...
```
*Where `UNIONVISOR_VERSION` is <Version chainId="union-testnet-5" genesis="false"/>*
...
```sh
docker run -v ~/.unionvisor:/.unionvisor -v /tmp:/tmp -it ghcr.io/unionlabs/bundle-testnet-5:$UNIONVISOR_VERSION init --moniker $MONIKER --network union-testnet-5 --seeds "daa56ac6573c6427ecef438d4c248d72b206597b@union.testnet.4.seed.poisonphang.com:26656"
```
...
services:
  node:
    image: ghcr.io/unionlabs/bundle-testnet-5:$UNIONVISOR_VERSION
    volumes:
