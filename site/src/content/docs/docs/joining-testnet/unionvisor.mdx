...
We release container images of Unionvisor called bundles. Each bundle contains everything required for running Unionvisor and joining a particular network. The Unionvisor bundle for <FunctionResult result={import.meta.env.CHAIN_ID_VERSION} asInlineCode /> is <FunctionResult result={import.meta.env.CHAIN_ID_VERSION.replace('union', 'bundle')} asInlineCode />. You can obtain the Unionvisor bundle for <FunctionResult result={import.meta.env.CHAIN_ID_VERSION} asInlineCode /> from our <FunctionResult result={`<a href="https://github.com/orgs/unionlabs/packages/container/package/${import.meta.env.CHAIN_ID_VERSION.replace('union', 'bundle')}">GitHub Container Registry</a>`} />.
...
<Code
  lang='sh'
  frame='code'
  code={`docker pull ghcr.io/unionlabs/${import.meta.env.CHAIN_ID_VERSION.replace('union', 'bundle')}:$UNIONVISOR_VERSION`} 
/>

_Where `UNIONVISOR_VERSION` is <FunctionResult result={chainVersion().current} />_
...
<Code
  ...
  code={dedent(`
    docker run \\
      ...
      --volume /tmp:/tmp \\
      -it ghcr.io/unionlabs/${import.meta.env.CHAIN_ID_VERSION.replace('union', 'bundle')}:$UNIONVISOR_VERSION \\
      init --moniker $MONIKER \\
      --network ${import.meta.env.CHAIN_ID_VERSION} \\
      --seeds "${chainVersion().seeds}"
  `)}
/>
...
- versions
  - <FunctionResult result={chainVersion().current} />- uniond
...
<Code
  lang='sh'
  frame='code'
  code={`alias uniond='docker run -v ~/.unionvisor:/.unionvisor -v /tmp:/tmp --network host -it ghcr.io/unionlabs/${import.meta.env.CHAIN_ID_VERSION.replace('union', 'bundle')}:$UNIONVISOR_VERSION --log-level off call --'`} 
/>
...
<Code
  ...
  services:
  node:
    image: ghcr.io/unionlabs/${import.meta.env.CHAIN_ID_VERSION.replace('union', 'bundle')}:$UNIONVISOR_VERSION
    volumes:
