...
For the Union Testnet, this value should be <FunctionResult result={'`"' + import.meta.env.CHAIN_ID_VERSION + '"`'} />.
...
Seed nodes help orchestrate initial connections to the network. For <FunctionResult result={'`' + import.meta.env.CHAIN_ID_VERSION + '`'} />, the seed nodes are:
