...
For the Union Testnet, this value should be <FunctionResult result={import.meta.env.CHAIN_ID_VERSION} asInlineCode />.

<Code
  lang='toml'
  code={dedent(`
    # The network chain ID
    chain-id = "${import.meta.env.CHAIN_ID_VERSION}"
  `)}
/>
...
Seed nodes help orchestrate initial connections to the network. For <FunctionResult result={import.meta.env.CHAIN_ID_VERSION} asInlineCode />, the seed nodes are:
