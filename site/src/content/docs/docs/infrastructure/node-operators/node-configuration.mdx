...
For the Union Testnet, this value should be <code><FunctionResult result={import.meta.env.CHAIN_ID_VERSION} /></code>.
...
Seed nodes help orchestrate initial connections to the network. For <code><FunctionResult result={import.meta.env.CHAIN_ID_VERSION} /></code>, the seed nodes are:
