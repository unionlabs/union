...
<Tabs>
  <TabItem label='x86_64-linux'>
    ...
    ```
    *Where `UNIOND_VERSION` is <FunctionResult result={chainVersion().current} />*
...
We'll need to set up a few configuration files and obtain the <DownloadFile source='external' link="https://rpc.testnet.bonlulu.uno/genesis" filename="genesis.json" /> before we can run the node.
...
<Code
  lang='bash'
  code={dedent(`
    export CHAIN_ID=${import.meta.env.CHAIN_ID_VERSION}
    export MONIKER="Unionized Goblin"
    export KEY_NAME=alice
    export GENESIS_URL="${getSeedsParts().address}:26657/genesis"
  `)}
/>
...
For <FunctionResult result={import.meta.env.CHAIN_ID_VERSION} asInlineCode /> replace `seeds = ""` with:

<Code
  lang='toml'
  code={`seeds = "${chainVersion().seeds}"`} 
/>
