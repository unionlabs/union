<script lang="ts">
  type PostMeta = {
    metadata: { title: string; date: string; author: string; preview: string; published: boolean }
  }
  const posts: Record<string, PostMeta> = import.meta.glob('../**/*.md', { eager: true })
...
<main class="flex flex-1 justify-center sm:pt-4">
    ...
    {#each filteredPosts as [path, { metadata }]}
      <a class="block p-4" href={extractSlug(path)}>
        ...
        <div class="font-mono mb-2 mt-1 sm:text-lg">
          {metadata.date} -
          <a class="text-accent" href={`https://x.com/${metadata.author}`}>@{metadata.author}</a>
        </div>
