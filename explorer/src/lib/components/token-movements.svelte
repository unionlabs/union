<script lang="ts">
import { extractTokenMovements, formatDenom, getMovementColor, type TokenMovementSummary } from "$lib/utils/tokens"
...
function getIcon(direction: string) {
  switch (direction) {
    ...
    case "burned": return FlameIcon
    case "minted": return SparklesIcon
    default: return ArrowUpRightIcon
  }
}
</script>
  ...
  <div class="relative border border-border">
    ...
    {#if uniqueDenoms.length > 0}
      <div class="p-4 border-b border-border bg-muted/10">
        <div class="text-[10px] font-mono uppercase tracking-wider text-muted-foreground mb-3">Net Flow</div>
    ...
    <div class="grid grid-cols-2 gap-px bg-border">
      ...
      <div class="bg-background p-4">
        <div class="flex items-center gap-2 mb-2">
          <ArrowUpRightIcon class="h-4 w-4 text-destructive" />
          <span class="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Total Sent</span>
        </div>
      ...
      <div class="bg-background p-4">
        <div class="flex items-center gap-2 mb-2">
          <ArrowDownLeftIcon class="h-4 w-4 text-success" />
          <span class="text-[10px] font-mono uppercase tracking-wider text-muted-foreground">Total Received</span>
        </div>
      ...
      <Collapsible.Root>
        <Collapsible.Trigger class="w-full">
          <div class="flex items-center gap-2 px-4 py-2.5 border-t border-border hover:bg-muted/20 transition-colors">
            ...
            <span class="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Per-Message Breakdown</span>
          </div>
        </Collapsible.Trigger>
        <Collapsible.Content>
          <div class="border-t border-border">
            {#each [...summary.perMessage.entries()].sort((a, b) => a[0] - b[0]) as [msgIdx, movements]}
              <div class="border-b border-border/50 last:border-b-0">
                <div class="px-4 py-2 bg-muted/10">
                  <span class="text-[10px] font-mono text-muted-foreground">Message {msgIdx + 1}</span>
                </div>
                <div class="divide-y divide-border/30">
                  {#each movements as movement}
                    ...
                    <div class="flex items-center gap-3 px-4 py-2 hover:bg-muted/5">
                      ...
                      <span class="text-[10px] font-mono uppercase w-16 shrink-0 {colorClass}">{movement.direction}</span>
                      <a href={urls.account(movement.address)} class="text-xs font-mono text-info hover:underline truncate">
                        {truncateAddress(movement.address, 8)}
                      </a>
                      <div class="flex items-baseline gap-1.5 ml-auto shrink-0">
                        <span class="font-mono text-sm {colorClass}">{formatAmount(movement.amount, 18)}</span>
                        <span class="text-[10px] text-muted-foreground font-mono">{formatDenom(movement.denom)}</span>
                      </div>
                      {#if movement.type === "cw20"}
                        <span class="text-[9px] px-1.5 py-0.5 bg-purple-500/10 text-purple-400 border border-purple-500/30">CW20</span>
                      {/if}
                    </div>
                  {/each}
                </div>
    ...
    {#if messageCount <= 1 && summary.movements.length > 0}
      <Collapsible.Root>
        <Collapsible.Trigger class="w-full">
          <div class="flex items-center gap-2 px-4 py-2.5 border-t border-border hover:bg-muted/20 transition-colors">
            <ChevronDownIcon class="h-3.5 w-3.5 text-muted-foreground shrink-0 transition-transform [[data-state=open]_&]:rotate-180" />
            <span class="text-[11px] font-medium uppercase tracking-wider text-muted-foreground">All Transfers</span>
