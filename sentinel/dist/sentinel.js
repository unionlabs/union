    ...
    try {
        ...
        for (let j = 0; j < 3; j++) {
            ...
            if (transferResp.isErr()) {
                consola.error("[%s] [%s->%s] Transfer error:", chainType, isCosmosChain ? task.sourceChainIdCosmos : task.sourceChainIdEVM, task.destinationChainId, transferResp.error);
                consola.info("retrying transfer");
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
            else {
                if (j === 0) {
                    consola.info("[%s] [%s->%s] Transfer success:", chainType, isCosmosChain ? task.sourceChainIdCosmos : task.sourceChainIdEVM, task.destinationChainId, transferResp.value);
                }
                else {
                    consola.info("[%s] [%s->%s] Transfer success after retry:", chainType, isCosmosChain ? task.sourceChainIdCosmos : task.sourceChainIdEVM, task.destinationChainId, transferResp.value);
                }
                break;
            }
        }
        ...
        consola.error("[%s] [%s->%s] Transfer exception: %s", isCosmosChain ? task.sourceChainIdCosmos : task.sourceChainIdEVM, task.destinationChainId, chainType, msg);
    }
}
/**
...
async function doTransferLoadTest(transfers, privKeys) {
    ...
    while (true) {
        ...
        counter += 1;
        // if (counter == 2){
        //   consola.info("counter is 2, brewaking.!!!!!!!!!!!!!!!!!!!!!1")
        //   break;
        // }
