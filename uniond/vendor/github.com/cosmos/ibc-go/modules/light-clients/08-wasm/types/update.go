...
func (cs ClientState) VerifyClientMessage(ctx context.Context, _ codec.BinaryCodec, clientStore storetypes.KVStore, clientMsg exported.ClientMessage) error {
...
func (cs ClientState) UpdateStateOnMisbehaviour(ctx context.Context, cdc codec.BinaryCodec, clientStore storetypes.KVStore, clientMsg exported.ClientMessage) {
	clientMessage, ok := clientMsg.(*ClientMessage)
	if !ok {
		panic(fmt.Errorf("expected type %T, got %T", &ClientMessage{}, clientMsg))
	}
