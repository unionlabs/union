...
contract IBCChannelTest is TestPlus {
    ...
    function test_openingHandshake_chainB(uint64 proofHeight, string memory portId) public {
        // 1. bindPort
        vm.assume(proofHeight > 0);
        (string memory clientId, string memory connId) = setupConnection_chainB(proofHeight);
        handler.bindPort(portId, address(app));
    ...
    function setupConnection_chainA(uint64 proofHeight) public returns (string memory clientId, string memory connId) {
        // 1. createClient
    ...
    function setupConnection_chainB(uint64 proofHeight) public returns (string memory clientId, string memory connId) {
        // 1. createClient
        IBCMsgs.MsgCreateClient memory m = MsgMocks.createClient(CLIENT_TYPE, proofHeight);
        clientId = handler.createClient(m);
