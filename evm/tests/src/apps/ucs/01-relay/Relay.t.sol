...
contract RelayTests is Test {
    ...
    function test_updateTokenMetadata_ok(
        ...
        TokenMetadata[] memory tokensMetadata = new TokenMetadata[](1);
        tokensMetadata[0].denom = args.denomName;
        tokensMetadata[0].name = updateArgs.name;
        tokensMetadata[0].symbol = updateArgs.symbol;
        tokensMetadata[0].decimals = updateArgs.decimals;
        ...
        vm.prank(address(ibcHandler));
        bytes memory ack = relay.onRecvPacket(
            IbcCoreChannelV1Packet.Data({
                sequence: args.sequence,
                source_port: args.sourcePort,
                source_channel: args.sourceChannel,
                destination_port: args.destinationPort,
                destination_channel: args.destinationChannel,
                data: UpdateMetadataPacketLib.encode(
        ...
        assertEq(ack, abi.encodePacked(RelayLib.ACK_SUCCESS));
