...
contract IBCPacketTest is TestPlus {
    ...
    function test_sendPacket() public {
        ClientHeight.Data memory timeoutHeight = ClientHeight.Data({
            revision_number: 0,
        ...
        assert(
            handler.commitments(
                IBCCommitment.packetCommitmentKey(portId, channelId, 1)
            ) != bytes32(0)
        );
    }
    ...
    function test_recvPacket() public {
        IBCMsgs.MsgPacketRecv memory msg_recv = MsgMocks.packetRecv(
            portId,
            channelId,
            proofHeight
        );
