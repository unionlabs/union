...
contract IBCTest is Test {
    ...
    function setUpConnection() internal {
        IbcCoreConnectionV1ConnectionEnd.Data
            memory connection = IbcCoreConnectionV1ConnectionEnd.Data({
    ...
    function setUpChannel() internal {
        ...
        hops[0] = "connection-0";
        IbcCoreChannelV1Channel.Data memory channel = IbcCoreChannelV1Channel
            .Data({
    ...
    function setUpMockApp() internal {
        ...
        handler.claimCapabilityDirectly(
            handler.channelCapabilityPath(portId, "channel-0"),
            address(mockApp)
        );
        handler.claimCapabilityDirectly(
            handler.channelCapabilityPath(portId, "channel-0"),
    ...
    function testConnectionOpenInit() public {
        IBCMsgs.MsgConnectionOpenInit memory msg_ = IBCMsgs
        ...
        handler.recvPacket(
            IBCMsgs.MsgPacketRecv({
                ...
                proof: abi.encodePacked(
                    sha256(abi.encodePacked(testPacketCommitment))
                ),
                proofHeight: IbcCoreClientV1Height.Data({
                    revision_number: 0,
                    revision_height: 1
                })
            })
                ...
                clientMessage: wrapAnyMockHeader(
                    IbcLightclientsMockV1Header.Data({
                        height: IbcCoreClientV1Height.Data({
                            revision_number: 0,
                            revision_height: next_revision_height
                        }),
                        timestamp: uint64(block.timestamp)
                    })
                )
    ...
    function getPacket()
        internal
        pure
        returns (IbcCoreChannelV1Packet.Data memory packet)
    {
        return
    ...
    function makePacketCommitment(
        IbcCoreChannelV1Packet.Data memory packet
