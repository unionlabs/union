...
library IbcLightclientsTendermintV1ClientState {
  ...
  function _read_unpacked_repeated_proof_specs(
     ...
     */
    (CosmosIcs23V1ProofSpec.Data memory x, uint256 sz) = _decode_CosmosIcs23V1ProofSpec(p, bs);
    if (isNil(r)) {
      counters[8] += 1;
    } else {
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1ProofSpec(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1ProofSpec.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1ProofSpec.Data memory r, ) = CosmosIcs23V1ProofSpec._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.proof_specs.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += CosmosIcs23V1ProofSpec._encode_nested(r.proof_specs[i], pointer, bs);
    }
    }
  ...
  function _estimate(
    ...
    for(i = 0; i < r.proof_specs.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1ProofSpec._estimate(r.proof_specs[i]));
    }
    for(i = 0; i < r.upgrade_path.length; i++) {
   ...
   */
  function addProofSpecs(Data memory self, CosmosIcs23V1ProofSpec.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    CosmosIcs23V1ProofSpec.Data[] memory tmp = new CosmosIcs23V1ProofSpec.Data[](self.proof_specs.length + 1);
