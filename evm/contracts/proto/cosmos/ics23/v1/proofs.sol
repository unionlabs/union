...
library CosmosIcs23V1ExistenceProof {
  ...
  function _read_leaf(
    ...
    (CosmosIcs23V1LeafOp.Data memory x, uint256 sz) = _decode_CosmosIcs23V1LeafOp(p, bs);
    r.leaf = x;
    return sz;
  }
  ...
  function _read_unpacked_repeated_path(
     ...
     */
    (CosmosIcs23V1InnerOp.Data memory x, uint256 sz) = _decode_CosmosIcs23V1InnerOp(p, bs);
    if (isNil(r)) {
      counters[4] += 1;
    } else {
   ...
   */
  function _decode_CosmosIcs23V1LeafOp(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1LeafOp.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1LeafOp.Data memory r, ) = CosmosIcs23V1LeafOp._decode(pointer, bs, sz);
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1InnerOp(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1InnerOp.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1InnerOp.Data memory r, ) = CosmosIcs23V1InnerOp._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.path.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += CosmosIcs23V1InnerOp._encode_nested(r.path[i], pointer, bs);
    }
    }
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1LeafOp._estimate(r.leaf));
    for(i = 0; i < r.path.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1InnerOp._estimate(r.path[i]));
    }
    return e;
  }
  // empty checker
   ...
   */
  function addPath(Data memory self, CosmosIcs23V1InnerOp.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    CosmosIcs23V1InnerOp.Data[] memory tmp = new CosmosIcs23V1InnerOp.Data[](self.path.length + 1);
...
library CosmosIcs23V1NonExistenceProof {
  ...
  function _read_left(
    ...
    (CosmosIcs23V1ExistenceProof.Data memory x, uint256 sz) = _decode_CosmosIcs23V1ExistenceProof(p, bs);
    r.left = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1ExistenceProof(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1ExistenceProof.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1ExistenceProof.Data memory r, ) = CosmosIcs23V1ExistenceProof._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += CosmosIcs23V1ExistenceProof._encode_nested(r.left, pointer, bs);
    ...
    pointer += CosmosIcs23V1ExistenceProof._encode_nested(r.right, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(r.key.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1ExistenceProof._estimate(r.left));
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1ExistenceProof._estimate(r.right));
    return e;
  }
  // empty checker
...
library CosmosIcs23V1CommitmentProof {
  ...
  function _read_nonexist(
    ...
    (CosmosIcs23V1NonExistenceProof.Data memory x, uint256 sz) = _decode_CosmosIcs23V1NonExistenceProof(p, bs);
    r.nonexist = x;
    return sz;
  }
  ...
  function _read_batch(
    ...
    (CosmosIcs23V1BatchProof.Data memory x, uint256 sz) = _decode_CosmosIcs23V1BatchProof(p, bs);
    r.batch = x;
    return sz;
  }
  ...
  function _read_compressed(
    ...
    (CosmosIcs23V1CompressedBatchProof.Data memory x, uint256 sz) = _decode_CosmosIcs23V1CompressedBatchProof(p, bs);
    r.compressed = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1NonExistenceProof(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1NonExistenceProof.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1NonExistenceProof.Data memory r, ) = CosmosIcs23V1NonExistenceProof._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1BatchProof(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1BatchProof.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1BatchProof.Data memory r, ) = CosmosIcs23V1BatchProof._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1CompressedBatchProof(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1CompressedBatchProof.Data memory, uint)
  {
    ...
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1CompressedBatchProof.Data memory r, ) = CosmosIcs23V1CompressedBatchProof._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += CosmosIcs23V1ExistenceProof._encode_nested(r.exist, pointer, bs);
    ...
    pointer += CosmosIcs23V1NonExistenceProof._encode_nested(r.nonexist, pointer, bs);
    ...
    pointer += CosmosIcs23V1CompressedBatchProof._encode_nested(r.compressed, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1ExistenceProof._estimate(r.exist));
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1NonExistenceProof._estimate(r.nonexist));
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1BatchProof._estimate(r.batch));
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1CompressedBatchProof._estimate(r.compressed));
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    CosmosIcs23V1CompressedBatchProof.store(input.compressed, output.compressed);
...
library CosmosIcs23V1LeafOp {
  ...
  function _read_hash(
    ...
    CosmosIcs23V1GlobalEnums.HashOp x = CosmosIcs23V1GlobalEnums.decode_HashOp(tmp);
    r.hash = x;
    return sz;
  }
  ...
  function _read_length(
    ...
    (int64 tmp, uint256 sz) = ProtoBufRuntime._decode_enum(p, bs);
    CosmosIcs23V1GlobalEnums.LengthOp x = CosmosIcs23V1GlobalEnums.decode_LengthOp(tmp);
    r.length = x;
    return sz;
  }
    ...
    returns (uint)
  {
    ...
    int32 _enum_prehash_key = CosmosIcs23V1GlobalEnums.encode_HashOp(r.prehash_key);
    ...
    int32 _enum_prehash_value = CosmosIcs23V1GlobalEnums.encode_HashOp(r.prehash_value);
    ...
    int32 _enum_length = CosmosIcs23V1GlobalEnums.encode_LengthOp(r.length);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_enum(CosmosIcs23V1GlobalEnums.encode_HashOp(r.hash));
    e += 1 + ProtoBufRuntime._sz_enum(CosmosIcs23V1GlobalEnums.encode_HashOp(r.prehash_key));
    e += 1 + ProtoBufRuntime._sz_enum(CosmosIcs23V1GlobalEnums.encode_HashOp(r.prehash_value));
    e += 1 + ProtoBufRuntime._sz_enum(CosmosIcs23V1GlobalEnums.encode_LengthOp(r.length));
...
library CosmosIcs23V1ProofSpec {
  ...
  function _read_inner_spec(
    ...
    (CosmosIcs23V1InnerSpec.Data memory x, uint256 sz) = _decode_CosmosIcs23V1InnerSpec(p, bs);
    r.inner_spec = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1InnerSpec(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1InnerSpec.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1InnerSpec.Data memory r, ) = CosmosIcs23V1InnerSpec._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += CosmosIcs23V1InnerSpec._encode_nested(r.inner_spec, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1LeafOp._estimate(r.leaf_spec));
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1InnerSpec._estimate(r.inner_spec));
...
library CosmosIcs23V1BatchProof {
  ...
  function _read_unpacked_repeated_entries(
     ...
     * if `r` is NULL, then only counting the number of fields.
     */
    (CosmosIcs23V1BatchEntry.Data memory x, uint256 sz) = _decode_CosmosIcs23V1BatchEntry(p, bs);
    if (isNil(r)) {
      counters[1] += 1;
    } else {
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1BatchEntry(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1BatchEntry.Data memory, uint)
  {
    uint256 pointer = p;
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1BatchEntry.Data memory r, ) = CosmosIcs23V1BatchEntry._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.entries.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += CosmosIcs23V1BatchEntry._encode_nested(r.entries[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.entries.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1BatchEntry._estimate(r.entries[i]));
    }
    return e;
  }
  // empty checker
   ...
   * @param value The value to add
   */
  function addEntries(Data memory self, CosmosIcs23V1BatchEntry.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    CosmosIcs23V1BatchEntry.Data[] memory tmp = new CosmosIcs23V1BatchEntry.Data[](self.entries.length + 1);
...
library CosmosIcs23V1CompressedBatchProof {
    ...
    returns (Data memory, uint)
  {
    ...
    if (counters[1] > 0) {
      require(r.entries.length == 0);
      r.entries = new CosmosIcs23V1CompressedBatchEntry.Data[](counters[1]);
    }
    if (counters[2] > 0) {
  ...
  function _read_unpacked_repeated_entries(
     ...
     * if `r` is NULL, then only counting the number of fields.
     */
    (CosmosIcs23V1CompressedBatchEntry.Data memory x, uint256 sz) = _decode_CosmosIcs23V1CompressedBatchEntry(p, bs);
    if (isNil(r)) {
      counters[1] += 1;
    } else {
      r.entries[r.entries.length - counters[1]] = x;
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1CompressedBatchEntry(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1CompressedBatchEntry.Data memory, uint)
  {
    ...
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1CompressedBatchEntry.Data memory r, ) = CosmosIcs23V1CompressedBatchEntry._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.entries.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += CosmosIcs23V1CompressedBatchEntry._encode_nested(r.entries[i], pointer, bs);
    }
    }
    ...
    for(i = 0; i < r.lookup_inners.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += CosmosIcs23V1InnerOp._encode_nested(r.lookup_inners[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
  ...
  function _estimate(
    ...
    uint256 e;uint256 i;
    for(i = 0; i < r.entries.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1CompressedBatchEntry._estimate(r.entries[i]));
    }
    for(i = 0; i < r.lookup_inners.length; i++) {
      e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1InnerOp._estimate(r.lookup_inners[i]));
    }
    return e;
  }
  // empty checker
   ...
   * @param value The value to add
   */
  function addEntries(Data memory self, CosmosIcs23V1CompressedBatchEntry.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    CosmosIcs23V1CompressedBatchEntry.Data[] memory tmp = new CosmosIcs23V1CompressedBatchEntry.Data[](self.entries.length + 1);
    for (uint256 i = 0; i < self.entries.length; i++) {
      tmp[i] = self.entries[i];
    }
    tmp[self.entries.length] = value;
   ...
   * @param value The value to add
   */
  function addLookupInners(Data memory self, CosmosIcs23V1InnerOp.Data memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    CosmosIcs23V1InnerOp.Data[] memory tmp = new CosmosIcs23V1InnerOp.Data[](self.lookup_inners.length + 1);
...
library CosmosIcs23V1CompressedBatchEntry {
  ...
  function _read_exist(
    ...
    (CosmosIcs23V1CompressedExistenceProof.Data memory x, uint256 sz) = _decode_CosmosIcs23V1CompressedExistenceProof(p, bs);
    r.exist = x;
    return sz;
  }
  ...
  function _read_nonexist(
    ...
    (CosmosIcs23V1CompressedNonExistenceProof.Data memory x, uint256 sz) = _decode_CosmosIcs23V1CompressedNonExistenceProof(p, bs);
    r.nonexist = x;
    return sz;
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1CompressedExistenceProof(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1CompressedExistenceProof.Data memory, uint)
  {
    ...
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1CompressedExistenceProof.Data memory r, ) = CosmosIcs23V1CompressedExistenceProof._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
   ...
   * @return The number of bytes used to decode
   */
  function _decode_CosmosIcs23V1CompressedNonExistenceProof(uint256 p, bytes memory bs)
    internal
    pure
    returns (CosmosIcs23V1CompressedNonExistenceProof.Data memory, uint)
  {
    ...
    (uint256 sz, uint256 bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
    (CosmosIcs23V1CompressedNonExistenceProof.Data memory r, ) = CosmosIcs23V1CompressedNonExistenceProof._decode(pointer, bs, sz);
    return (r, sz + bytesRead);
  }
    ...
    returns (uint)
  {
    ...
    pointer += CosmosIcs23V1CompressedExistenceProof._encode_nested(r.exist, pointer, bs);
    ...
    pointer += CosmosIcs23V1CompressedNonExistenceProof._encode_nested(r.nonexist, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1CompressedExistenceProof._estimate(r.exist));
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1CompressedNonExistenceProof._estimate(r.nonexist));
    return e;
  }
  // empty checker
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    CosmosIcs23V1CompressedNonExistenceProof.store(input.nonexist, output.nonexist);
...
library CosmosIcs23V1CompressedNonExistenceProof {
    ...
    returns (uint)
  {
    ...
    pointer += CosmosIcs23V1CompressedExistenceProof._encode_nested(r.left, pointer, bs);
    ...
    pointer += CosmosIcs23V1CompressedExistenceProof._encode_nested(r.right, pointer, bs);
  ...
  function _estimate(
    ...
    uint256 e;
    e += 1 + ProtoBufRuntime._sz_lendelim(r.key.length);
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1CompressedExistenceProof._estimate(r.left));
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosIcs23V1CompressedExistenceProof._estimate(r.right));
    return e;
  }
  // empty checker
