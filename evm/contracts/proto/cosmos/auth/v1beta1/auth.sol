...
library CosmosAuthV1beta1ModuleCredential {
    ...
    returns (Data memory, uint)
  {
    ...
    while (pointer < offset + sz) {
      ...
      if (fieldId == 2) {
        pointer += _read_unpacked_repeated_derivation_keys(pointer, bs, nil(), counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    while (pointer < offset + sz) {
      ...
      pointer += bytesRead;
      if (fieldId == 2) {
        pointer += _read_unpacked_repeated_derivation_keys(pointer, bs, r, counters);
      } else
      {
        pointer += ProtoBufRuntime._skip_field_decode(wireType, pointer, bs);
      }
    ...
    returns (uint)
  {
    ...
    for(i = 0; i < r.derivation_keys.length; i++) {
      pointer += ProtoBufRuntime._encode_key(
        ...
        bs)
      ;
      pointer += ProtoBufRuntime._encode_bytes(r.derivation_keys[i], pointer, bs);
    }
    }
    return pointer - offset;
  }
   ...
   * @param value The value to add
   */
  function addDerivationKeys(Data memory self, bytes memory value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
