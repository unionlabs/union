...
library Ics23  {
    ...
    // verifyMembership, throws an exception in case anything goes wrong
        ...
        if (CosmosIcs23V1ExistenceProof.isNil(exiProof)) return VerifyMembershipError.ExistenceProofIsNil;
    ...
    enum VerifyNonMembershipError {
        ...
        if (CosmosIcs23V1NonExistenceProof.isNil(nonProof)) return VerifyNonMembershipError.NonExistenceProofIsNil;
    ...
    // private
    function getExistProofForKey(CosmosIcs23V1CommitmentProof.Data memory proof, bytes memory key) private pure returns(CosmosIcs23V1ExistenceProof.Data memory) {
        if (CosmosIcs23V1ExistenceProof.isNil(proof.exist) == false){
            ...
            for (uint i = 0; i < proof.batch.entries.length; i++) {
                if (CosmosIcs23V1ExistenceProof.isNil(proof.batch.entries[i].exist) == false &&
    ...
    function getNonExistProofForKey(CosmosIcs23V1CommitmentProof.Data memory proof, bytes memory key) private pure returns(CosmosIcs23V1NonExistenceProof.Data memory) {
        if (CosmosIcs23V1NonExistenceProof.isNil(proof.nonexist) == false) {
            ...
            for (uint i = 0; i < proof.batch.entries.length; i++) {
                if (CosmosIcs23V1NonExistenceProof.isNil(proof.batch.entries[i].nonexist) == false &&
    ...
    function isLeft(CosmosIcs23V1ExistenceProof.Data memory left, bytes memory key) private pure returns(bool) {
        // CosmosIcs23V1ExistenceProof.isNil does not work
        return CosmosIcs23V1ExistenceProof._empty(left) || Ops.compare(left.key, key) < 0;
    }

    function isRight(CosmosIcs23V1ExistenceProof.Data memory right, bytes memory key) private pure returns(bool) {
        // CosmosIcs23V1ExistenceProof.isNil does not work
        return CosmosIcs23V1ExistenceProof._empty(right) || Ops.compare(right.key, key) > 0;
    }
}

library Compress {
    function decompress(CosmosIcs23V1CommitmentProof.Data memory proof) internal pure returns(CosmosIcs23V1CommitmentProof.Data memory) {
    ...
    // private
    function decompress(CosmosIcs23V1CompressedBatchProof.Data memory proof) private pure returns(CosmosIcs23V1BatchEntry.Data[] memory) {
        CosmosIcs23V1BatchEntry.Data[] memory entries = new CosmosIcs23V1BatchEntry.Data[](proof.entries.length);
    ...
    function decompressEntry(CosmosIcs23V1CompressedBatchEntry.Data memory entry, CosmosIcs23V1InnerOp.Data[] memory lookup) private pure returns(CosmosIcs23V1BatchEntry.Data memory) {
        //CosmosIcs23V1ExistenceProof.isNil does not work
    ...
    function decompressExist(CosmosIcs23V1CompressedExistenceProof.Data memory proof, CosmosIcs23V1InnerOp.Data[] memory lookup) private pure returns(CosmosIcs23V1ExistenceProof.Data memory) {
        if (CosmosIcs23V1CompressedExistenceProof._empty(proof)) {
            return CosmosIcs23V1ExistenceProof.nil();
        }
        CosmosIcs23V1ExistenceProof.Data memory decoProof = CosmosIcs23V1ExistenceProof.Data({
            key: proof.key,
            value: proof.value,
            leaf: proof.leaf,
...
library Ops {
    ...
    // LeafOp operations
        function applyOp(CosmosIcs23V1LeafOp.Data memory leafOp, bytes memory key, bytes memory value) internal pure returns(bytes memory, ApplyLeafOpError) {
    ...
    // preapare leaf data for encoding
        function prepareLeafData(CosmosIcs23V1GlobalEnums.HashOp hashOp, CosmosIcs23V1GlobalEnums.LengthOp lenOp, bytes memory data) internal pure returns(bytes memory, PrepareLeafDataError) {
    ...
    enum CheckAgainstSpecError{
        ...
        function checkAgainstSpec(CosmosIcs23V1LeafOp.Data memory leafOp, CosmosIcs23V1ProofSpec.Data memory spec) internal pure returns(CheckAgainstSpecError) {
    ...
    // InnerOp operations
        function applyOp(CosmosIcs23V1InnerOp.Data memory innerOp, bytes memory child ) internal pure returns(bytes memory, ApplyInnerOpError) {
    ...
    function checkAgainstSpec(CosmosIcs23V1InnerOp.Data memory innerOp, CosmosIcs23V1ProofSpec.Data memory spec) internal pure returns(CheckAgainstSpecError) {
    ...
    function doHashOrNoop(CosmosIcs23V1GlobalEnums.HashOp hashOp, bytes memory preImage) internal pure returns(bytes memory, DoHashError) {
    ...
    enum DoHashError {
        ...
        function doHash(CosmosIcs23V1GlobalEnums.HashOp hashOp, bytes memory preImage) internal pure returns(bytes memory, DoHashError) {
    ...
    enum DoLengthOpError {
        ...
        function doLengthOp(CosmosIcs23V1GlobalEnums.LengthOp lenOp, bytes memory data) private pure returns(bytes memory, DoLengthOpError) {
...
library Proof{
    ...
    // ExistenceProof
        function verify(CosmosIcs23V1ExistenceProof.Data memory proof, CosmosIcs23V1ProofSpec.Data memory spec, bytes memory commitmentRoot,bytes memory key, bytes memory value) internal pure returns(VerifyExistenceError) {
    ...
    function calculateRoot(CosmosIcs23V1ExistenceProof.Data memory proof) internal pure returns(bytes memory, CalculateRootError) {
        //require(LeafOp.isNil(proof.leaf) == false); // dev: Existence Proof needs defined LeafOp
        if (CosmosIcs23V1LeafOp.isNil(proof.leaf)) return (empty, CalculateRootError.LeafNil);
    ...
    enum CheckAgainstSpecError{
        ...
        function checkAgainstSpec(CosmosIcs23V1ExistenceProof.Data memory proof, CosmosIcs23V1ProofSpec.Data memory spec) internal pure returns(CheckAgainstSpecError) {
        // LeafOp.isNil does not work
        //require(LeafOp._empty(proof.leaf) == false); // dev: Existence Proof needs defined LeafOp
        if (CosmosIcs23V1LeafOp._empty(proof.leaf)) return CheckAgainstSpecError.EmptyLeaf;
    ...
    function verify(CosmosIcs23V1NonExistenceProof.Data memory proof, CosmosIcs23V1ProofSpec.Data memory spec, bytes memory commitmentRoot,bytes memory key) internal pure returns(VerifyNonExistenceError) {
    ...
    function calculateRoot(CosmosIcs23V1NonExistenceProof.Data memory proof) internal pure returns(bytes memory, CalculateRootError) {
        if (CosmosIcs23V1ExistenceProof._empty(proof.left) == false) {
    ...
    // commitment proof
    function calculateRoot(CosmosIcs23V1CommitmentProof.Data memory proof) internal pure returns(bytes memory, CalculateRootError) {
        ...
        if (CosmosIcs23V1BatchProof._empty(proof.batch) == false) {
            ...
            if (CosmosIcs23V1BatchEntry._empty(proof.batch.entries[0])) return (empty, CalculateRootError.BatchEntryEmpty);
            if (CosmosIcs23V1ExistenceProof._empty(proof.batch.entries[0].exist) == false) {
                return calculateRoot(proof.batch.entries[0].exist);
            }
            if (CosmosIcs23V1NonExistenceProof._empty(proof.batch.entries[0].nonexist) == false) {
    ...
    // private
    function isLeftMost(CosmosIcs23V1InnerSpec.Data memory spec, CosmosIcs23V1InnerOp.Data[] memory path) private pure returns(bool) {
    ...
    function isRightMost(CosmosIcs23V1InnerSpec.Data memory spec, CosmosIcs23V1InnerOp.Data[] memory path) private pure returns(bool){
    ...
    function isLeftStep(CosmosIcs23V1InnerSpec.Data memory spec, CosmosIcs23V1InnerOp.Data memory left, CosmosIcs23V1InnerOp.Data memory right) private pure returns(bool){
    ...
    function isLeftNeighbor(CosmosIcs23V1InnerSpec.Data memory spec, CosmosIcs23V1InnerOp.Data[] memory left, CosmosIcs23V1InnerOp.Data[] memory right) private pure returns(bool) {
    ...
    enum OrderFromPaddingError {
        ...
        function orderFromPadding(CosmosIcs23V1InnerSpec.Data memory spec, CosmosIcs23V1InnerOp.Data memory op) private pure returns(uint, OrderFromPaddingError) {
    ...
    enum GetPaddingError {
        ...
        function getPadding(CosmosIcs23V1InnerSpec.Data memory spec, uint branch) private pure returns(uint minPrefix, uint maxPrefix, uint suffix, GetPaddingError) {
    ...
    function hasPadding(CosmosIcs23V1InnerOp.Data memory op, uint minPrefix, uint maxPrefix, uint suffix) private pure returns(bool) {
        if (op.prefix.length < minPrefix) return false;
        if (op.prefix.length > maxPrefix) return false;
        return op.suffix.length == suffix;
    }

    function sliceInnerOps(CosmosIcs23V1InnerOp.Data[] memory array, uint start, uint end) private pure returns(CosmosIcs23V1InnerOp.Data[] memory) {
        CosmosIcs23V1InnerOp.Data[] memory slice = new CosmosIcs23V1InnerOp.Data[](end-start);
