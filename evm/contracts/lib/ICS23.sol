...
library Ics23 {
    ...
    function verifyMembership(
        ...
        CosmosIcs23V1CommitmentProof.Data memory decoProof = Compress
            .decompress(proof);
        CosmosIcs23V1ExistenceProof.Data memory exiProof = getExistProofForKey(
            decoProof,
            key
        );
    ...
    function verifyNonMembership(
        ...
        CosmosIcs23V1NonExistenceProof.Data
            memory nonProof = getNonExistProofForKey(decoProof, key);
    ...
    function isLeft(
        ...
        return
            CosmosIcs23V1ExistenceProof._empty(left) ||
    ...
    function isRight(
        ...
        // CosmosIcs23V1ExistenceProof.isNil does not work
        return
            CosmosIcs23V1ExistenceProof._empty(right) ||
...
library Compress {
    function decompress(
        CosmosIcs23V1CommitmentProof.Data memory proof
    ) internal pure returns (CosmosIcs23V1CommitmentProof.Data memory) {
        //CosmosIcs23V1CompressedBatchProof.isNil() does not work
        if (
            CosmosIcs23V1CompressedBatchProof._empty(proof.compressed) == true
        ) {
            return proof;
        }
        return
    ...
    // private
    function decompress(
        CosmosIcs23V1CompressedBatchProof.Data memory proof
    ) private pure returns (CosmosIcs23V1BatchEntry.Data[] memory) {
        CosmosIcs23V1BatchEntry.Data[]
            memory entries = new CosmosIcs23V1BatchEntry.Data[](
    ...
    function decompressEntry(
        ...
        //CosmosIcs23V1ExistenceProof.isNil does not work
        if (
            CosmosIcs23V1CompressedExistenceProof._empty(entry.exist) == false
        ) {
            return
                CosmosIcs23V1BatchEntry.Data({
    ...
    function decompressExist(
        ...
        CosmosIcs23V1ExistenceProof.Data
            memory decoProof = CosmosIcs23V1ExistenceProof.Data({
                key: proof.key,
                value: proof.value,
                leaf: proof.leaf,
...
library Ops {
    ...
    function applyOp(
        ...
        (bytes memory pKey, PrepareLeafDataError pCode1) = prepareLeafData(
            leafOp.prehash_key,
            leafOp.length,
            key
        );
    ...
    function applyOp(
        ...
        (bytes memory hashed, DoHashError code) = doHash(
            innerOp.hash,
            preImage
        );
    ...
    function checkAgainstSpec(
        ...
        if (innerOp.hash != spec.inner_spec.hash)
            return CheckAgainstSpecError.Hash;
        uint256 minPrefixLength = SafeCast.toUint256(
            spec.inner_spec.min_prefix_length
        );
        ...
        uint256 maxLeftChildBytes = (spec.inner_spec.child_order.length - 1) *
            childSize;
        uint256 maxPrefixLength = SafeCast.toUint256(
            spec.inner_spec.max_prefix_length
        );
...
library Proof {
    ...
    function calculateRoot(
        ...
        (bytes memory root, Ops.ApplyLeafOpError lCode) = Ops.applyOp(
            proof.leaf,
            proof.key,
            proof.value
        );
    ...
    function checkAgainstSpec(
        ...
        Ops.CheckAgainstSpecError cCode = Ops.checkAgainstSpec(
            proof.leaf,
            spec
        );
        ...
        if (spec.min_depth > 0) {
            bool innerOpsDepthTooShort = proof.path.length <
                SafeCast.toUint256(int256(spec.min_depth));
        ...
        if (spec.max_depth > 0) {
            bool innerOpsDepthTooLong = proof.path.length >
                SafeCast.toUint256(int256(spec.max_depth));
    ...
    function calculateRoot(
        ...
        if (CosmosIcs23V1BatchProof._empty(proof.batch) == false) {
            ...
            if (
                CosmosIcs23V1ExistenceProof._empty(
                    proof.batch.entries[0].exist
                ) == false
            ) {
        ...
        if (
            CosmosIcs23V1CompressedBatchProof._empty(proof.compressed) == false
        ) {
    ...
    function isLeftStep(
        ...
        (uint leftIdx, OrderFromPaddingError lCode) = orderFromPadding(
            spec,
            left
        );
        if (lCode != OrderFromPaddingError.None) return false;
        (uint rightIdx, OrderFromPaddingError rCode) = orderFromPadding(
            spec,
            right
        );
        if (lCode != OrderFromPaddingError.None) return false;
    ...
    function getPadding(
        ...
        private
        pure
        returns (uint minPrefix, uint maxPrefix, uint suffix, GetPaddingError)
    {
        uint uChildSize = SafeCast.toUint256(spec.child_size);
        (uint idx, GetPositionError gCode) = getPosition(
            spec.child_order,
            branch
        );
    ...
    function getPosition(
        int32[] memory order,
        uint branch
    ) private pure returns (uint, GetPositionError) {
    ...
    function sliceInnerOps(
        ...
        CosmosIcs23V1InnerOp.Data[]
            memory slice = new CosmosIcs23V1InnerOp.Data[](end - start);
