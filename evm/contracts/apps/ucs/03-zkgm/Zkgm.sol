...
abstract contract UCS03ZkgmStore is IZkgmStore {
    ...
    mapping(uint32 => mapping(uint256 => mapping(address => uint256))) public
        channelBalance;
    mapping(uint32 => IZkgmERC20) public channelGovernanceToken;
    ...
    UCS03ZkgmStore
{
    ...
    string internal constant STAKE_NFT_NAME =
        "Union Cross-Chain Staking Position";
    string internal constant STAKE_NFT_SYMBOL = "USP";
    ...
    function _stakingFailed(uint32 channelId, Stake calldata _stake) internal {
        ...
        IZkgmERC20 governanceToken = channelGovernanceToken[channelId];
    ...
    function _withdrawSucceeded(
        ...
        _getStakeNFTManager().burn(tokenId);
    }
    ...
    function registerGovernanceToken(
        ...
        if (channelGovernanceToken[channelId] != IZkgmERC20(address(0))) {
            revert ZkgmLib.ErrChannelGovernanceTokenAlreadySet();
        }
        channelGovernanceToken[channelId] = token;
    }
    ...
    function stake(
        ...
        IZkgmERC20 governanceToken = channelGovernanceToken[channelId];
        if (governanceToken == IZkgmERC20(address(0))) {
        ...
        uint256 tokenId = _mintNFT(channelId, amount);
    ...
    function acknowledgeWithdrawStake(
        ...
        if (successful) {
            _withdrawSucceeded(_withdrawStake.tokenId);
        } else {
