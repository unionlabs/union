...
mod tests {
    ...
    fn serde() {
        ...
        let expected_msg = RestrictedExecuteMsg::Send(WhenNotPaused::wrap(SendMsg::Send {
        ...
        let msg = serde_json_wasm::from_str::<RestrictedExecuteMsg>(json_str).unwrap();

        assert_eq!(msg, expected_msg);

        let msg = serde_json_wasm::from_str::<ExecuteMsg>(json_str).unwrap();

        assert_eq!(msg, ExecuteMsg::Restricted(Restricted::wrap(expected_msg)));
    }
}
