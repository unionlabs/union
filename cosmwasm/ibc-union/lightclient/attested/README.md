...
Note that all attestations contain a height and a timestamp. Rather than having a second attestation flow for height timestamps (which are required by the ibc union light client specification), the timestamps for a height are derived from the attestations for that height. Given this, once an attestation exists at a height, all future attestations at that same height must contain the same timestamp.  
...
This contract fully supports the ibc union light client specification, as follows:
...
This is largely just a stub implementation, and no actual verifications are performed, other than ensuring that The provided height is attested to the provided timestamp. Both of these values can be queried directly from the contract, so anyone can permissionlessly update the client once an attestation has been confirmed. This functionality enables this client to be transparently supported by the ibc union stack. 
