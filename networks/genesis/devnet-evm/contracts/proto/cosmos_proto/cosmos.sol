...
library Cosmos_protoScalarDescriptor {
  ...
  function _read_unpacked_repeated_field_type(
    ...
    CosmosGlobalEnums.ScalarType x = CosmosGlobalEnums.decode_ScalarType(tmp);
    if (isNil(r)) {
      counters[3] += 1;
    } else {
    ...
    returns (uint)
  {
    ...
    pointer += ProtoBufRuntime._encode_varint(
      CosmosGlobalEnums.estimate_packed_repeated_ScalarType(r.field_type),
      pointer,
      bs
    );
    for(i = 0; i < r.field_type.length; i++) {
      int32 _enum_field_type = CosmosGlobalEnums.encode_ScalarType(r.field_type[i]);
  ...
  function _estimate(
    ...
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.name).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(bytes(r.description).length);
    e += 1 + ProtoBufRuntime._sz_lendelim(CosmosGlobalEnums.estimate_packed_repeated_ScalarType(r.field_type));
    return e;
  }
  // empty checker
   ...
   */
  function addFieldType(Data memory self, CosmosGlobalEnums.ScalarType  value) internal pure {
    /**
     * First resize the array. Then add the new element to the end.
     */
    CosmosGlobalEnums.ScalarType[] memory tmp = new CosmosGlobalEnums.ScalarType[](self.field_type.length + 1);
