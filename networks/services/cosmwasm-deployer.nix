...
let
  ...
  cosmwasm-deployer = pkgs.writeShellApplication {
    name = "cosmwasm-deployer";
    runtimeInputs = [ node ];
    text =
      if builtins.length instantiations == 0 then
        ""
      else
        ''
          mkdir -p /tmp
          ...
          ${pkgs.lib.getExe node} \
            ...
            sign-batch ${
              builtins.concatStringsSep " " (
                builtins.genList (id: "./msg${builtins.toString id}.json") (builtins.length instantiations)
              )
            } \
            --append \
            --home ${devnet-home} \
