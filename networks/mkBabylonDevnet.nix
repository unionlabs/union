...
let
  ...
  mkBlsKey = idx:
    assert (builtins.isInt idx);
    pkgs.runCommand
      "${chainName}-valbls_${toString idx}"
      {
        buildInputs = [ pkgs.jq ];
      ...
      ''
        ...
        tmpHome=$(mktemp -d)
        cp -r --no-preserve=mode $src/* $tmpHome

        ${nodeBin} keys list --keyring-backend test
  ...
  devnet-home = pkgs.runCommand "${chainName}-home" { } ''
    ...
    ${genScriptForEachVal (idx: ''
      cat ${mkBlsKey idx}
      ${nodeBin} gen-helpers add-bls ${mkBlsKey idx} --home .
    '')}
