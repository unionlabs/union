<script lang="ts">
  ...
  $: {
    ...
    console.info(
      JSON.stringify($page.url.searchParams.get("timestamp"), undefined, 2)
    )
   ...
   */
  let REFETCH_ENABLED = writable(
    $page.url.searchParams.has("timestamp") ? false : true
  )
  ...
  let queryStatus: "pending" | "done" = $REFETCH_ENABLED
    ? $liveTransfers.status === "pending" ||
      $liveTransfers.fetchStatus === "fetching"
      ? "pending"
      : "done"
  ...
  const columns: Array<ColumnDef<DataRow, TransferAddress>> = [
    {
      ...
      minSize: 200,
      maxSize: 200,
      accessorFn: (originalRow, index) => originalRow.source,
      cell: (_info) => {
        ...
        const chainDisplayName =
          info.chains.find((chain) => chain.chain_id === chainId)
    ...
    },
    {
      ...
      maxSize: 200,
      cell: (_info) => {
        const info = _info as CellContext<DataRow, DataRow> & {
          chains: Array<Chain>
        }
        // @ts-expect-error
        const { chainId, address } = info.getValue()
