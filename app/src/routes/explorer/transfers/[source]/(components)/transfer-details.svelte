<script lang="ts">
    ...
    ([$transfers, $submittedTransfers]) => {
      ...
      return $transfers.data.map((transfer) => {
        ...
        return {
          ...
          // @ts-ignore
          transfer_day: toIsoString(new Date(tx.source_timestamp)).split(
            "T"
          )[0],
          ...tx
        }
  ...
  let tracesSteps: Readable<Array<Array<Step>> | null> = derived(
    ...
    ([$processedTraces, $processedTransfers]) => {
      ...
      return zip($processedTransfers, $processedTraces).map(
        ([transfer, traces]) => {
          const onSourceTrace = (eventType: string) =>
            traces.find(
              (t) =>
                t.type === eventType &&
                t.chain?.chain_id === transfer.source_chain_id
            )
          const onSource = (eventType: string) =>
            onSourceTrace(eventType) !== undefined
          const onHopTrace = (eventType: string) =>
            traces.find(
              (t) =>
                t.type === eventType &&
                t.chain?.chain_id === transfer.hop_chain_id
            )
          const onHop = (eventType: string) =>
            onHopTrace(eventType) !== undefined
          const onDestinationTrace = (eventType: string) =>
            traces.find(
              (t) =>
                t.type === eventType &&
                t.chain?.chain_id === transfer.destination_chain_id
            )
          const onDestination = (eventType: string) =>
            onDestinationTrace(eventType) !== undefined
            ...
            return [
              ...
              (() => {
                ...
                return {
                  ...
                  description:
                    status === "IN_PROGRESS"
                      ? `Waiting on ${sourceChainName} finality`
                      : "",
          ...
          return [
            ...
            (() => {
              ...
              return {
                ...
                description:
                  status === "IN_PROGRESS"
                    ? `Waiting on ${hopChainName} finality`
                    : "",
