...
let ucs01Configuration = derived(
  ...
  ([$fromChain, $toChainId, $recipient]) => {
    if ($fromChain === null || $toChainId === null || !$recipient) return null
      ...
      <Card.Content class={cn('flex flex-col gap-4')}>
        ...
        <section>
          ...
          {#if $fromChain}
            ...
            {/if}
          {:else}
            Please select chain to send from
          {/if}
...
<!--            </Button>-->
    ...
    <div class="cube-left font-bold flex items-center justify-center text-xl font-supermolot">UNION UNION UNION
      UNION
