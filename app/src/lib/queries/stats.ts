...
export const packetCountQuery = () =>
  createQuery({
        ...
        queryFn: async () => (await request(URLS.GRAPHQL, packetCountQueryDocument, {})).v0_packets_aggregate,
        enabled: true,
        refetchInterval: 6_000,
        refetchOnWindowFocus: false
  })
...
export const transferCountQuery = () =>
  createQuery({
        queryKey: ["packet-count"],
        queryFn: async () => (await request(URLS.GRAPHQL, transferCountQueryDocument, {})).v0_transfers_aggregate,
        enabled: true,
        refetchInterval: 6_000,
        refetchOnWindowFocus: false
  })