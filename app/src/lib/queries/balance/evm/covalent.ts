...
export async function getBalancesFromCovalent({
  address,
  chainId
}: { address: string; chainId: keyof typeof covalentChainMap }): Promise<
  Array<{
    name: string
    symbol: string
    address: string
...
> {
  ...
  const response = await fetch(
    `https://api.covalenthq.com/v1/${covalentChainMap[chainId]}/address/${address}/balances_v2/?${searchParams.toString()}`,
    {
      method: "GET",
      headers: {
