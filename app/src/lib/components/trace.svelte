...
const NON_ACK_KEYS = [
  ...
  "PACKET_SEND_LC_UPDATE_L1",
  "PACKET_SEND_LC_UPDATE_L2"
] as const

export const filteredDisplayNames = derived(showDetailedTrace, $showAcknowledgement => ({
  ...Object.fromEntries(
    Object.entries(DISPLAY_NAMES).map(([status, statusNames]) => [
      status,
      Object.fromEntries(
        Object.entries(statusNames).filter(
          ([key]) =>
            $showAcknowledgement || NON_ACK_KEYS.includes(key as (typeof NON_ACK_KEYS)[number])
        )
      )
    ])
  )
...
$: console.log($filteredDisplayNames)
...
$: pTraces = ((): Array<Trace> => {
  ...
  // First filter out non-PACKET traces if showAcknowledgement is false
  processedTraces = processedTraces.filter(
    trace =>
      $showDetailedTrace || NON_ACK_KEYS.includes(trace.type as (typeof NON_ACK_KEYS)[number])
  )
  ...
  return processedTraces.map(t => ({
    ...t,
    type: $filteredDisplayNames[t.status][t.type] ?? t.type
  }))
})()
