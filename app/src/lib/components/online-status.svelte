...
let blockHeightStore = $derived(blockHeightQuery())
let blockHeight = $derived($blockHeightStore.data)
let appStatus = $derived(Number.isSafeInteger(Number(blockHeight)) ? "online" : "offline")
</script>

<Tooltip.Root>
  <Tooltip.Trigger  class="backdrop-blur-sm ml-1 mb-16 md:mb-1 my-a fixed bottom-0 z-50 w-18">
