...
let confirmedUserAddr: Readable<UserAddresses> = derived(userAddr, $userAddr => {
  if ($userAddr === null) {
    ...
    return {
      cosmos: { canonical: "never", normalized: "never", bytes: new Uint8Array([]), normalized_prefixed: "0x0" as Address },
      evm: { canonical: "0xnever" as Address, normalized: "never", normalized_prefixed: "0x0" as Address }
    }
  }
  return $userAddr
})
