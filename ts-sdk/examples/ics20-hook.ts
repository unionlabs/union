...
export const makeIcs20PacketMemo = async (
  ...
  Effect.gen(function*() {
    ...
    const instruction: Ucs03Ng.Root = yield* ZkgmInstruction.encodeNg(tokenOrder)
    const packet: Ucs03Ng.ZkgmPacket = Ucs03Ng.ZkgmPacket.make({
      path: 0n,
      salt: salt,
      instruction,
    })
    const hash: `0x${string}` = yield* Ucs03Ng.zkgmPacketToHash({
      sourceChannelId: SOURCE_CHANNEL_ID,
      destinationChannelId: ChannelId.make(0),
      timeoutTimestamp: timeout_timestamp,
      packet,
    })
    yield* Effect.log(`Packet Hash: ${hash}`)
    return {
      wasm: {
