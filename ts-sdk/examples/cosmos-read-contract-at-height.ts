...
Effect.runPromiseExit(
  Effect.gen(function* () {
    ...
    const balanceAtBlock123 = yield* channelBalanceAtHeight(
        0n,
        "ubbn",
        912421
      ).pipe(
      ...
      const withExtClient = Effect.provideService(ExtendedCosmWasmClientContext, { client: extClient });
      const cw20BalanceAtHeight = yield* readCw20BalanceAtHeight(
        contractAddr,
        minter,
        912421
      ).pipe(withExtClient)
