...
impl<T: FetcherClient> Indexer<T> {
    pub async fn run_publisher(&self) -> Result<(), IndexerError> {
        loop {
            match self.run_publisher_loop().await {
                ...
                Err(error) => {
                    warn!(
                        "error in finalizer loop: {error} => try again later (sleep {}ms)",
    ...
    async fn run_publisher_loop(&self) -> Result<PublisherLoopResult, IndexerError> {
        if let Some(nats) = &self.nats {
            ...
            let messages =
                next_to_publish(&mut tx, &subject, self.publisher_config.batch_size).await?;

            if messages.is_empty() {
                info!("nothing scheduled to send => retry later");
