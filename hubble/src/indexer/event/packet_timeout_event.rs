...
mod tests {
    ...
    /// Creates a test PacketTimeoutEvent with predictable values
    fn create_test_event(suffix: u32) -> PacketTimeoutEvent {
        let header = create_test_header(suffix);
        let (channel_id, packet_hash, maker, _maker_msg, _acknowledgement) =
