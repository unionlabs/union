...
mod tests {
    ...
    /// Creates a test ConnectionOpenInitEvent with unique deterministic values
    fn create_test_event(suffix: u32) -> ConnectionOpenInitEvent {
        let header = create_test_header(suffix);
        let (connection_id, client_id, counterparty_client_id, _counterparty_connection_id) =
