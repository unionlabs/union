...
impl<'a> TryFrom<&'a EventContext<'a, ChainContext, CreateLensClientEvent>>
    for CreateLensClientRecord
{
    type Error = IndexerError;

    fn try_from(
        value: &'a EventContext<'a, ChainContext, CreateLensClientEvent>,
...
impl CreateLensClientRecord {
    pub async fn insert(&self, tx: &mut Transaction<'_, Postgres>) -> Result<(), IndexerError> {
        ...
        sqlx::query!(
            r#"
            INSERT INTO v2_sync.create_lens_client_sync (
                internal_chain_id,
                block_hash,
                height,
                timestamp,
    ...
    pub async fn delete_by_chain_and_height(
        ...
        let result = sqlx::query!(
            r#"
            DELETE FROM v2_sync.create_lens_client_sync
