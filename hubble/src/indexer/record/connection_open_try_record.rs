...
use crate::indexer::{
    api::IndexerError,
    event::{
        connection_open_try_event::ConnectionOpenTryEvent,
...
impl<'a> TryFrom<&'a InternalChainIdContext<'a, ConnectionOpenTryEvent>>
    for ConnectionOpenTryRecord
{
    type Error = IndexerError;

    fn try_from(
        value: &'a InternalChainIdContext<'a, ConnectionOpenTryEvent>,
...
impl ConnectionOpenTryRecord {
    pub async fn insert(&self, tx: &mut Transaction<'_, Postgres>) -> Result<(), IndexerError> {
        ...
        sqlx::query!(
            r#"
            INSERT INTO v2_sync.connection_open_try_test (
                internal_chain_id,
                block_hash,
                height,
                timestamp,
    ...
    pub async fn delete_by_chain_and_height(
        ...
        let result = sqlx::query!(
            r#"
            DELETE FROM v2_sync.connection_open_try_test
