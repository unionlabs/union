...
use crate::indexer::{
    api::IndexerError,
    event::{types::BlockHeight, write_ack_event::WriteAckEvent},
...
impl HasKind for WriteAckRecord {
    fn kind() -> RecordKind {
        RecordKind::WriteAck
    }
}

impl<'a> TryFrom<&'a EventContext<'a, ChainContext, WriteAckEvent>> for WriteAckRecord {
    type Error = IndexerError;

    fn try_from(
        value: &'a EventContext<'a, ChainContext, WriteAckEvent>,
        ...
        Ok(Self {
            ...
            acknowledgement: value.event.acknowledgement.pg_value()?,
...
impl WriteAckRecord {
    pub async fn insert(
        ...
        sqlx::query!(
            r#"
            INSERT INTO v2_sync.write_ack_sync (
                internal_chain_id,
                block_hash,
                height,
                event_index,
    ...
    pub async fn delete_by_chain_and_height(
        ...
        let result = sqlx::query!(
            r#"
            DELETE FROM v2_sync.write_ack_sync
