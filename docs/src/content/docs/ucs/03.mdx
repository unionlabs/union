...
- `instruction`: The [Instruction](#instruction) to forward
  - Can be Call, FungibleAssetOrder, or Batch
  - Will be executed on final destination chain
  - Cannot be another Forward instruction

:::tip

The forward instruction can wrap Call, FungibleAssetOrder, or Batch instructions. The path parameter encodes pairs of (previous destination channel, next source channel) for routing. For example, to forward through a single hop, you need both the previous destination channel ID and the next source channel ID that form the connection between chains.
...
- `instructions`: Array of [Instructions](#instruction) to execute atomically
  - Only specific instructions allowed (Call, FungibleAssetOrder, Stake, Unstake, WithdrawStake)
...
struct TokenOrderV2 {
    ...
    bytes baseToken;          // Token being sent
    uint256 baseAmount;       // Amount being sent
    uint8 kind;       // Type of metadata (image, preimage, image_unwrap)
...
3. Solver Fill - Smart contracts that implement the solver interface can programmatically fill orders:
   ...
   - On failure, solvers return `ACK_ERR_ONLYMAKER` to allow other market makers to fill
