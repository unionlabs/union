---
title: Upgrade History
sidebar:
  order: 10
---

import { Badge } from "@astrojs/starlight/components";

Here you can find records of Union network binary upgrades.

## union-testnet-10

The network `union-testnet-10` has had three binary upgrades since its genesis.

| Upgrade                                                                    | Height  |
| -------------------------------------------------------------------------- | ------- |
| [v1.0.0](https://github.com/unionlabs/union/releases/tag/uniond%2Fv1.0.0)  | genesis |
| [v1.1.1](https://github.com/unionlabs/union/releases/tag/uniond%2Fv1.1.1)  | 1380400 |
| [v1.2.0](https://github.com/unionlabs/union/releases/tag/uniond%2Fv1.2.0)  | 2004000 |
| [v1.2.1*](https://github.com/unionlabs/union/releases/tag/uniond%2Fv1.2.1) | 2084158 |

:::note
\* patch version to fix consensus error
:::

## union-testnet-9

<Badge text="Deprecated" variant="caution" />

The network `union-testnet-9` has had no binary upgrades since its genesis.

| Upgrade | Height  |
| ------- | ------- |
| v0.25.0 | genesis |

## union-testnet-8

<Badge text="Deprecated" variant="caution" />

The network `union-testnet-8` had one binary upgrade since its genesis.

| Upgrade | Height  |
| ------- | ------- |
| v0.21.0 | genesis |
| v0.22.0 | 186500  |

## union-testnet-7

<Badge text="Deprecated" variant="caution" />

The network `union-testnet-7` had no binary upgrades since its genesis.

| Upgrade | Height  |
| ------- | ------- |
| v0.20.0 | genesis |

## union-testnet-6

<Badge text="Deprecated" variant="caution" />

The network `union-testnet-6` had no binary upgrades since its genesis.

| Upgrade | Height  |
| ------- | ------- |
| v0.19.0 | genesis |

## union-testnet-5

<Badge text="Deprecated" variant="caution" />

The network `union-testnet-5` had no binary upgrades since its genesis.

| Upgrade | Height  |
| ------- | ------- |
| v0.18.0 | genesis |

## union-testnet-4

<Badge text="Deprecated" variant="caution" />

The network `union-testnet-4` had two binary upgrades since its genesis.

| Upgrade | Height  |
| ------- | ------- |
| v0.14.0 | genesis |
| v0.15.0 | 236668  |
| v0.17.0 | 631650  |
