...
Instsll CLI packages: `ripgrep` and `jq` <Badge text="Optional" type="warning" class='ml-2' />

```sh frame="none"
cargo add ripgrep jq cowsay
```
...
The above will returun a transaction hash at the end, use it t query the transaction to get the `code_id`:
...
The above will returun a transaction hash at the end, use it t query the transaction to get the `_contract_address`:
...
Let's construct the JSON payload for the contract execution. For the `receiver` field we are using Vitalik's address:

```json title="payload.json"
{
  "transfer": {
    "receiver": "6053a7794e8aCe9be62808828003fFFE2710BBBC",
    "amount": "1",
    "denom": "muno",
