...
Install the `uniond` binary which you will use to publish the contract and execute transfer
...
Install CLI packages: `ripgrep` and `jaq` <Badge text="Optional" type="warning" class='ml-2' />

```sh frame="none"
cargo add ripgrep jaq
```
...
The above will return a transaction hash at the end, use it t query the transaction to get the `code_id`:
