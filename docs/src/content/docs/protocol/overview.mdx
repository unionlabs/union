...
flowchart TB
    Connection --> Channel
    Channel --> ICS20-1-relay
    Channel --> UCS0-1-relay
    Channel --> UCS0-2-relay
    Channel --> UCS0-3-relay
`}/>

Each protocol version has different functionalities and intended use cases. For most users, `ucs03-relay-1` is the appropriate channel type.

| version       | use case                                          |
|---------------|--------------------------------------------------|
| ucs01-relay-1 | Asset transfers                                  |
| ucs02-relay-1 | NFT transfers                                    |
| ucs03-relay-1 | Asset transfers, NFT transfers, packets, intents |
