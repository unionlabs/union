...
Finally, the connection ceremony can begin.


<Steps>
1. To initiate the ceremony, send a `connection_open_init` message from Stargaze to Union.
    ```sh
    ./starsd tx wasm execute stars1s0x3yq0pmltxq56f4yppgmd02ret3uj5k9ftj6ug9c7lc379sw7qv396zm '{"client_id":"","counterparty_client_id":"","relayer":""}' \
      --home ./union-home \
      --node $UNION_RPC_URL \
      --chain-id union-testnet-9 \
      --from $KEY_NAME \
      ...
      -y
    ```
2. This will trigger a sequence of events that will eventually complete the connection ceremony.
</Steps>
