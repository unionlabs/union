...
const DECREMENT_IN_SUB_MSG: RoleId = RoleId::new(4);

const INCREMENT_GUARDIAN: RoleId = RoleId::new(5);
const DECREMENT_GUARDIAN: RoleId = RoleId::new(6);
...
async fn main() -> Result<()> {
    ...
    wait_for_block_with(&client, |block| {
...
async fn wait_for_block_with(
    rpc: &cometbft_rpc::Client,
    f: impl Fn(BlockResponse) -> bool,
) -> Result<()> {
    loop {
        let block = rpc.block(None).await?;
        if f(block) {
            return Ok(());
        }
    }
