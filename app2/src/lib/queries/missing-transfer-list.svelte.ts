  ...
  return Effect.gen(function* () {
    ...
    while (true) {
      ...
      const page = yield* fetchDecodeGraphql(
        ...
        missingAckDoc,
        { limit, sortOrder: cursor, src: source, dst: destination }
      )
    ...
    console.info("Missing transfers:", found, " between chains: ", source, " -> ", destination)
    return found
  })
}
...
export function runInWindowAllPairs() {
  ...
  const allPairs = Object.entries(TIMEFRAMES)
  const allEffects = allPairs.map(([pair]) => {
    const [source, destination] = pair.split("|")
    return transferListInWindow(source, destination, 100)
  })
