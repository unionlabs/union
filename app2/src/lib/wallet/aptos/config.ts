...
export function createAptosStore(
  ...
  const { subscribe, set, update } = persisted("aptos-store", previousState, {
    syncTabs: true,
    storage: "session"
  })
...
aptosStore.subscribe($aptosStore => {
  if ($aptosStore?.address) {
    ...
    wallets.aptosAddress = Option.some(aptosAddressFromHex($aptosStore.address))
  } else {
