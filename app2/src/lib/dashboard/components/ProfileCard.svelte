      ...
      <div class="flex flex-col gap-2">
        <div class="flex items-start justify-between">
          ...
          <button
            ...
            aria-label="Logout"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-zinc-400 group-hover:text-red-500 transition-colors">
                  ...
                  {#if Option.isSome(dashboard.experience.value.current)}
                    <div class="flex flex-col text-end">
                      {#if Option.isSome(dashboard.experience.value.next)}
                        <span class="text-neutral-400">→ {dashboard.experience.value.next.value.title}</span>
                      {/if}
