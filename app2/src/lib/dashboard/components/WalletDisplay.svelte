<script lang="ts">
  ...
  function getGroupColorClass(groupingId: string | null): string {
    if (!groupingId) return "border-transparent";
  ...
  function truncateGroupingId(groupingId: string | null): string {
    if (!groupingId) return '-'; // Display dash if no group
    if (groupingId.length <= 11) return groupingId;
    ...
    Option.match(dashboard.wallets, {
      ...
      onSome: (walletStore) =>
        Option.match(walletStore.wallets, {
          onNone: () => [],
          onSome: (_w) => groupWalletsForDisplay(
            walletStore.enhanced.filter((w: EnhancedWallet) => getWalletCategory(w.chain_id) === 'evm')
          )
        })
    })
  );
    ...
    Option.match(dashboard.wallets, {
      ...
      onSome: (walletStore) =>
        Option.match(walletStore.wallets, {
          onNone: () => [],
          onSome: (_w) => groupWalletsForDisplay(
            walletStore.enhanced.filter((w: EnhancedWallet) => getWalletCategory(w.chain_id) === 'cosmos')
          )
        })
    })
  );
    ...
    Option.match(dashboard.wallets, {
      ...
      onSome: (walletStore) =>
        Option.match(walletStore.wallets, {
          onNone: () => [],
          onSome: (_w) => groupWalletsForDisplay(
            walletStore.enhanced.filter((w: EnhancedWallet) => getWalletCategory(w.chain_id) === 'other')
          )
        })
    })
  );
  ...
  function handleRemoveSelectedWallets() {
    if (Option.isNone(dashboard.wallets) || selectedWallets.size === 0 || İsRemoving) return;
    İsRemoving = true;
    ...
    Effect.runPromise(
      pipe(
        ...
        Effect.ensuring(Effect.sync(() => { İsRemoving = false; }))
      )
    );
  }
    ...
    <div class="flex flex-col gap-3">
      ...
      {:else if evmWalletGroups.length > 0}
        {#each evmWalletGroups as walletGroup (walletGroup[0].address + walletGroup[0].chain_id)} 
          <Card class="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4 transition-all duration-200 hover:border-zinc-600/50">
              ...
              {#each walletGroup as wallet (wallet.address + wallet.chain_id)}
                <div class="flex items-center justify-between gap-3 pt-1 first:pt-0">
                  <div class="min-w-0 flex-grow flex items-center">
                    <span class="font-mono text-sm text-zinc-300 truncate block" title={wallet.address}>{truncateAddress(wallet.address)}</span>
                  </div>
                  <label class="flex items-center cursor-pointer shrink-0">
                    ...
                    >
                      {#if selectedWallets.has(`${wallet.address}::${wallet.chain_id}`)}
                        <svg class="w-2.5 h-2.5 text-accent pointer-events-none" viewBox="0 0 16 16" fill="currentColor"><path d="M12.207 4.793a1 1 0 010 1.414l-5 5a1 1 0 01-1.414 0l-2-2a1 1 0 011.414-1.414L6.5 9.086l4.293-4.293a1 1 0 011.414 0z"/></svg>
                      {/if}
                    </span>
                  </label>
                </div>
  ...
  <Card class="flex flex-col">
    ...
    <div class="flex flex-col gap-3">
      ...
      {:else if cosmosWalletGroups.length > 0}
        {#each cosmosWalletGroups as walletGroup (walletGroup[0].grouping || walletGroup[0].address + walletGroup[0].chain_id)} 
          <Card class="bg-zinc-900/50 border border-zinc-800 rounded-lg p-4 transition-all duration-200 hover:border-zinc-600/50">
            <div class="flex flex-col gap-2">
              {#each walletGroup as wallet (wallet.address + wallet.chain_id)}
                <div class="flex items-center justify-between gap-3 pt-1 first:pt-0">
