...
import { Effect, Match, Option } from "effect" // For handling Option type from WalletStore
...
import {
  ...
  type StateResult,
} from "../services/add-cosmos-wallet"
...
let selectedChains: Array<string | null> = $state([])
let message: string = $state("")
let hasError = $state(false)
let currentState: AddEvmWalletState | AddCosmosWalletState | null = $state(null)
...
const connectedEvmWallet = $derived.by(() => {
  if (
    ...
    return evmWalletsInformation.find((wallet: { id: string }) =>
...
function selectCosmosAndSetDefaults() {
  page = "cosmos"
  selectedChains = cosmosChains.map(chain => chain.chain_id)
}
      ...
      Match.when("cosmos-verify", () =>
        Effect.gen(function*() {
          ...
          if (Option.isNone(targetChain)) {
            message = `Cosmos chain 'union-testnet-9' not found in configuration.`
            hasError = true
            return
          }

          currentState = AddCosmosWalletState.SwitchChain({ chain: targetChain.value })
          message = "Initiating Cosmos wallet verification..."
          hasError = false

          while (currentState !== null) {
            const result: StateResult = yield* addCosmosWallet(currentState, selectedChains)
...
const primaryButtonText = $derived(
  Match.value<[PageState, boolean]>([page, hasError]).pipe(
    ...
    Match.when(["completed", true], () => "Done"),
  ...
  <div class="flex-1 min-h-0 flex flex-col">
    ...
    {:else if page === "evm"}
      <div class="relative min-w-full flex flex-col flex-1 h-full min-h-0">
        <div class="grow flex flex-col gap-2 p-4 flex-1 min-h-0">
          <p class="text-sm text-zinc-400">
            To add your EVM wallet to your dashboard, you need to verify ownership by signing a
            message. This is a one-time verification for this wallet.
          </p>
    ...
    {:else if page === "cosmos-verify"}
      <div class="relative min-w-full flex flex-col flex-1 h-full min-h-0">
        <div class="grow flex flex-col gap-2 p-4 flex-1 min-h-0">
          <p class="text-sm text-zinc-400">
            To add your Cosmos wallet to your dashboard, you need to verify ownership by signing a
            message. This is a one-time verification for this wallet.
          </p>
    ...
    <!-- Unified Bottom Bar -->
    {#if page !== "select"}
      <!-- Show button bar for all pages except 'select' -->
      <div class="border-t border-zinc-800 bg-zinc-925">
        <div class="flex justify-between p-4">
          ...
          {/if}
          {#if showSecondaryCloseButton}
            <!-- Specifically for "Close" on completed page -->
            <Button
              variant="secondary"
              onclick={() => {
                isOpen = false
