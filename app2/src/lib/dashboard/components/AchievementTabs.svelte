<script lang="ts">
  ...
  let availableCategories = $derived(
    Option.flatMap(dashboard.achievements, (achievements) => 
      Option.flatMap(achievements.available, (availableAchievements) => 
        Option.some([...new Set(availableAchievements.map(a => a.category?.title ?? ''))])
      )
    )
  );
  ...
  let subcategoryCounts = $derived(
    Option.flatMap(dashboard.achievements, (achievements) => 
      Option.flatMap(achievements.available, (availableAchievements) => {
        const counts: Record<string, number> = {};
        for (const achievement of availableAchievements.filter(a => a.category?.title === selectedCategory)) {
  ...
  function getAchievedAchievements(achievements: { achieved?: Option.Option<UserAchievement[]> } | null): UserAchievement[] {
  ...
  let achievementChains = $derived(
    Option.flatMap(dashboard.achievements, (achievements) => 
      Option.flatMap(achievements.available, (availableAchievements) => {
        ...
        if (selectedCategory) {
          filteredAchievements = filteredAchievements.filter(a => a.category?.title === selectedCategory);
        }
        
        // Filter by subcategory if selected
        if (selectedSubcategory) {
          filteredAchievements = filteredAchievements.filter(a => a.subcategory?.title === selectedSubcategory);
        }
        ...
        const findChainStart = (achievement: AchievementType): AchievementType => {
          const previous = filteredAchievements.find((a: AchievementType) => a.next === achievement.id);
          return previous ? findChainStart(previous) : achievement;
        };

        const chains = filteredAchievements.reduce((chains: AchievementType[][], achievement: AchievementType) => {
          // Skip if achievement is already in any chain
          if (chains.some((chain: AchievementType[]) => chain.some((a: AchievementType) => a.id === achievement.id))) {
            return chains;
          }
          ...
          while (current) {
            ...
            const nextAchievement: AchievementType | null = nextId !== null ? achievementsMap.get(nextId) ?? null : null;
  ...
  let totalXP = $derived(
    Option.flatMap(dashboard.achievements, (achievements) => 
      Option.flatMap(achievements.available, (availableAchievements) => 
        Option.some(availableAchievements.reduce((sum: number, a: AchievementType) => sum + (a.xp || 0), 0))
      )
    )
  );
      ...
      Option.flatMap(achievements.achieved, (achievedAchievements) => 
        Option.flatMap(achievements.available, (availableAchievements) => 
          Option.some(achievedAchievements.reduce((sum: number, ua: UserAchievement) => {
            const achievement = availableAchievements.find((a: AchievementType) => a.id === ua.achievement_id);
  ...
  let progress = $derived(
    Option.flatMap(totalXP, (total) => {
      ...
      return Option.flatMap(earnedXP, (earned) => 
        Option.some(Math.min((earned / total) * 100, 100))
      );
    })
  );
  ...
  function getChainEarnedXP(chain: AchievementType[], userAchievements: UserAchievement[]): number {
    return chain.reduce((sum, a) => {
      ...
      if (!userAchievement || userAchievement.progression === undefined || userAchievement.threshold === undefined) {
        return sum;
      }
...
<Card class="flex flex-col gap-4">
  <div class="flex flex-col gap-4">
    ...
    <div class="flex items-center gap-2 text-sm overflow-x-auto whitespace-nowrap no-scrollbar p-2">
      ...
      >
        All ({Option.getOrNull(Option.flatMap(dashboard.achievements, (achievements) => 
          Option.flatMap(achievements.available, (availableAchievements) => 
      ...
      {:else}
        ...
        {:else}
          ...
          <span class="font-medium text-white capitalize">
            {selectedSubcategory} ({Option.getOrNull(subcategoryCounts)?.[selectedSubcategory] ?? 0})
          </span>
        {/if}
      {/if}
    </div>
      ...
      {#if Option.isSome(dashboard.achievements)}
        {#if getOptionArrayLength(achievementChains) > 0}
          {#each Option.getOrNull(sortedChains) ?? [] as chain, chainIndex}
            {@const organizedChain = organizeChain(chain, getAchievedAchievements(Option.getOrNull(dashboard.achievements)))}
            {@const isExpanded = expandedChains.has(chainIndex)}
            {@const chainTotalXP = getChainTotalXP(chain)}
            {@const chainEarnedXP = getChainEarnedXP(chain, getAchievedAchievements(Option.getOrNull(dashboard.achievements)))}
            {@const chainProgress = getChainProgress(chain, getAchievedAchievements(Option.getOrNull(dashboard.achievements)))}
            
            <Card class="flex flex-col gap-2 p-4 bg-zinc-900/50 border border-zinc-800 {chainProgress === 100 ? 'border-accent/20' : ''} transition-all duration-200 hover:border-zinc-600/50">
                ...
                <div class="flex items-center gap-2">
                  <div class="relative">
                    <div class="px-1.5 py-0.5 rounded-sm bg-zinc-800/80 border border-zinc-700/50 {chainProgress === 100 ? 'scale-110 border-accent/50' : ''} transition-all duration-300 flex items-center justify-center">
                      <span class="text-[10px] lg:text-xs font-medium {chainProgress === 100 ? 'text-accent' : 'text-white'} transition-all duration-300">
              ...
              {#if isExpanded}
                {@const availableRewards = chain.flatMap(a => a.reward_achievements?.map(r => r.rewards) ?? []).filter(r => {
                  if (!r || !Array.isArray(r)) return false;
                  return r.some(reward => !reward.cutoff || new Date(reward.cutoff) > new Date());
                })}
                {@const earnedRewards = organizedChain.completed.flatMap(a => a.reward_achievements?.map(r => r.rewards) ?? []).filter(r => {
                  if (!r || !Array.isArray(r)) return false;
                  return r.some(reward => !reward.cutoff || new Date(reward.cutoff) > new Date());
                })}
          ...
          <div class="absolute inset-0 flex items-center justify-center z-10 w-full h-full">
            <div class="bg-zinc-900/1 backdrop-blur-sm w-full h-full flex items-center justify-center">
              <div class="text-center">
                ...
                <div class="text-sm text-zinc-500">Please wait while we fetch your achievements</div>
              </div>
            </div>
          </div>
          <div class="flex flex-col gap-4">
