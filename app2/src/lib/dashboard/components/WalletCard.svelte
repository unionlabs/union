...
<Card class="flex flex-col gap-3 p-4 transition-all hover:bg-zinc-800/50 group">
  <div class="flex items-center justify-between gap-2">
    <div class="flex items-center gap-3 min-w-0">
      ...
      {#if category === 'evm'}
        <span class="text-sm font-medium text-zinc-200 truncate" title={getChainDisplayNameForTitle(wallet.chain_id, category)}>
          {getChainDisplayNameForTitle(wallet.chain_id, category)}
        </span>
      {/if}
      {#if wallet.hasGrouping}
        <span title="Part of a wallet group" class="text-zinc-500 text-xs whitespace-nowrap shrink-0">
          (Grouped)
        </span>
      {/if}
    </div>
  ...
  <div class="text-xs text-zinc-400 font-mono break-all" title={wallet.address}>
    {truncateAddress(wallet.address)}
  </div>

  <div class="text-xs text-zinc-500 mt-1">
    Added: {new Date(wallet.createdAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
  </div>
</Card> 