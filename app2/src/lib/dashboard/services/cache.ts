import { Effect, Option, pipe, Data } from "effect";
...
export function getLastGlobalSync(locale: string = "default"): Effect.Effect<Option.Option<{ label: string; timestamp: number }>, CacheError, never> {
  return pipe(
          ...
          onSome: (raw) =>
            pipe(
              ...
              Effect.flatMap((timestamp) => {
                if (isNaN(timestamp)) return Effect.succeed(Option.none());
