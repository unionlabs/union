...
onMount(() => {
  try {
    ...
    if (savedRecent) {
      recentAddresses = JSON.parse(savedRecent)
    }

    const savedBookmarked = localStorage.getItem("bookmarkedAddresses")
    if (savedBookmarked) {
      bookmarkedAddresses = JSON.parse(savedBookmarked)
    }
  } catch (e) {
...
{#if open}
  <div
    class="absolute bg-zinc-925 inset-0 z-40"
  ...
  >
    ...
    >
      ...
      >
        ...
        {#if (currentView === "recent" && hasRecent())
          || (currentView === "bookmarks" && hasBookmarks())}
          <div class="flex items-center h-full">
            <button
        ...
        <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
          <div class="bg-zinc-800 p-4 rounded-lg w-4/5 max-w-md">
            <h4 class="text-lg font-semibold mb-3">
              Clear all {currentView === "recent" ? "recent" : "bookmarked"}
              addresses?
            </h4>
