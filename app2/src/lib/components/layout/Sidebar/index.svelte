    ...
    <div class="flex flex-col flex-1">
      ...
      {#each navigation as section, i}
        {#if section.title !== "Developer" || uiStore.showDeveloperPages}
          <section
            class="border-zinc-900 p-6 last:flex-1 flex flex-col justify-end"
          >
            {#if section.title}
              <h2
                class="font-bold text-sm mb-2.5 text-center uppercase text-zinc-600"
              >
                {section.title}
              </h2>
            {/if}
              ...
              {#each section.items as item}
                <li>
                  ...
                  {#if item.subroutes && item.subroutes.length > 0}
                    <ul
                      class="flex flex-col border-zinc-800 gap-1 pt-2 border-l-1 ml-5 pl-2"
                    >
