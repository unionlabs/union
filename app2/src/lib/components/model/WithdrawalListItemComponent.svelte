...
import type { Withdrawal } from "@unionlabs/sdk/schema/stake"
...
const handleClick = () => {
  goto(`/explorer/withdrawals/${item.packet_hash}`)
}

const status = $derived(
  Option.isSome(item.withdraw_recv_timestamp)
    ? "success"
  ...
  >
    ...
    <div class="flex items-center gap-2">
      <DateTimeComponent
        ...
        value={item.withdraw_send_timestamp}
        showSeconds={false}
      />
