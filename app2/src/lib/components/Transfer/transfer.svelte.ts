...
import {QuoteData, type Token, WethTokenData} from "$lib/schema/token.ts";
...
export class Transfer {
  ...
  baseTokens = $derived.by<readonly Token[]>(() => {
  ...
  channel = $derived.by<Option.Option<Channel>>(() => {
    if (!Option.isSome(channels.data) ||
      !Option.isSome(this.sourceChain) ||
      !Option.isSome(this.destinationChain)) {
      return Option.none();
    }
  ...
  ucs03address = $derived.by<Option.Option<Address>>(() => {
    ...
    const hexAddress: Hex = sourceChain.rpc_type === "cosmos"
  ...
  getW = async () => {
    ...
    if (Option.isNone(this.destinationChain)) console.log("[wethQuoteToken] Missing destinationChain");
  ...
  args = $derived.by(() => {
    ...
    const wethQuoteTokenValue = Option.isSome(this.wethQuoteToken) ? this.wethQuoteToken.value : null;

    return {
      sourceChain: sourceChainValue ? getChainFromWagmi(Number(sourceChainValue.chain_id)) as ViemChain : null,
