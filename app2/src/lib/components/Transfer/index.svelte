<script lang="ts">
  ...
  $effect(() => {
    ...
    const machineEffect = Effect.gen(function* () {
      ...
      let finalAllowances: Array<{ token: string; requiredAmount: string; currentAllowance: string }> = [];
      ...
      steps.push(
        ...finalAllowances
          .filter(({ requiredAmount, currentAllowance }) => BigInt(currentAllowance) < BigInt(requiredAmount))
