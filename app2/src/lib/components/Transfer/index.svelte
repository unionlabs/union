<script lang="ts">
  ...
  let instruction: Option.Option<Instruction.Instruction> = $state(
    Option.none(),
  );
  let allowances: Option.Option<Array<{ token: string; allowance: bigint }>> =
    $state(Option.none());
  ...
  let requiredApprovals = $derived.by(() => {
    if (Option.isNone(transferIntents) || Option.isNone(allowances))
      return Option.none();
    ...
    return tokensNeedingApproval.length > 0
      ? Option.some(tokensNeedingApproval)
      : Option.none();
  });
  ...
  let transferSteps = $derived.by(() => {
    ...
    if (Option.isSome(requiredApprovals)) {
      ...
      for (const approval of requiredApprovals.value) {
        if (Option.isSome(allowances)) {
          const tokenAllowance = allowances.value.find(
            (a) => a.token === approval.token,
          );
          if (tokenAllowance) {
            steps.push(
    ...
    if (Option.isSome(instruction)) {
      steps.push(
        TransferStep.SubmitInstruction({ instruction: instruction.value }),
      );
  ...
  const intentsToBatch = (ti: typeof transferIntents) =>
    Effect.gen(function* () {
      ...
      const cosmosChannelDestinationEffect = Effect.succeed({
        ucs03address: fromHex(
          transfer.channel.value.destination_port_id,
          "string",
        ),
        channelId: transfer.channel.value.destination_channel_id,
      });
        ...
        const orders = yield* Match.value([source, destination]).pipe(
          Match.when(["evm", "cosmos"], () => {
            ...
            return Effect.all([
              Effect.tap(
                ...
                (order) =>
                  Effect.sync(() =>
                    console.log("batch: First order created", order),
                  ),
              ),
              Effect.tap(
                ...
                (order) =>
                  Effect.sync(() =>
                    console.log("batch: Second order created", order),
                  ),
              ),
            ]).pipe(
              Effect.tap((orders) =>
                Effect.sync(() =>
                  console.log("batch: All orders created", orders),
                ),
              ),
          ...
          Match.when(["evm", "evm"], () => {
            ...
            return Effect.all([
              ...
              Effect.tap((orders) =>
                Effect.sync(() =>
                  console.log("batch: EVM->EVM orders created", orders),
                ),
              ),
              Effect.catchAll((error) => {
                console.error(
                  "batch: Error creating EVM->EVM orders",
                  error.cause,
                );
                return Effect.fail(error);
              }),
          ...
          Match.when(["cosmos", "evm"], () => {
            ...
            return createCosmosToEvmFungibleAssetOrder(ti.value[0]).pipe(
              Effect.tap((order) =>
                Effect.sync(() =>
                  console.log("batch: Cosmos->EVM order created", order),
                ),
              ),
              Effect.catchAll((error) => {
          ...
          Match.when(["cosmos", "cosmos"], () => {
            ...
            return createCosmosToCosmosFungibleAssetOrder(ti.value[0]).pipe(
              Effect.tap((order) =>
                Effect.sync(() =>
                  console.log("batch: Cosmos->Cosmos order created", order),
                ),
              ),
              Effect.catchAll((error) => {
                console.error(
                  "batch: Error creating Cosmos->Cosmos order",
                  error.cause,
                );
                return Effect.fail(error);
              }),
          ...
          Match.orElse((x) => {
            console.log(
              `batch: No match found for ${source} -> ${destination}, throwing error`,
            );
            throw new Error(
              `Unsupported source/destination combination: ${source} -> ${destination}`,
            );
          }),
        );
  ...
  const checkAllowances = (ti: typeof transferIntents) =>
    Effect.gen(function* () {
      ...
      const tokenAddresses = [
        ...new Set(ti.value.map((intent) => intent.baseToken)),
      ];
          ...
          tokenAddresses.map((tokenAddress) =>
            Effect.gen(function* () {
              ...
              if (!isValidBech32ContractAddress(decodedAddr)) {
                console.log(
                  "It's native token, returning none. Token:",
                  tokenAddress,
                );
                return Option.none();
              }
              ...
              Effect.tapErrorCause((cause) =>
                Effect.logError("Predict failed with cause", cause),
              ),
            ),
          ),
        );
