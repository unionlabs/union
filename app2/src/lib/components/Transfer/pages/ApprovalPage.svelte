<script lang="ts">
  ...
  import type { ApprovalRequired } from "../transfer-step.ts"
  ...
  const { stepIndex, step, onBack, onApprove, actionButtonText }: Props = $props()
  ...
  const submit = Effect.gen(function* () {
    ...
    try {
      const chain = step.context.sourceChain
            ...
            do {
              ets = yield* Effect.promise(() =>
                nextStateEvm(ets, viemChain.value, publicClient, walletClient, {
                  ...
                  args: [step.context.ucs03address, step.requiredAmount]
                })
              )
