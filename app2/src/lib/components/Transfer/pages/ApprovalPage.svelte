<script lang="ts">
  ...
  const step = $derived(
    lts.pipe(
      ...
      Option.flatMap(Arr.findFirst(is("ApprovalRequired"))),
    ),
  );

  const sourceChain = $derived(lts.pipe(Option.map(Struct.get("sourceChain"))));

  let ets = $state<TransactionSubmissionEvm>(
    TransactionSubmissionEvm.Filling(),
  );
  let cts = $state<TransactionSubmissionCosmos>(
    TransactionSubmissionCosmos.Filling(),
  );
      ...
      yield* Match.value(sourceChainRpcType).pipe(
        Match.when("evm", () =>
          Effect.gen(function* () {
            ...
            do {
              ...
              if (
                ets._tag === "SwitchChainComplete" ||
                ets._tag === "WriteContractComplete"
              ) {
        ...
        Match.when("cosmos", () =>
          Effect.gen(function* () {
            ...
            const sender = yield* lts.value.sourceChain.getDisplayAddress(
              wallets.cosmosAddress.value,
            );
            const spender =
              cosmosSpenderAddresses[lts.value.sourceChain.universal_chain_id];
            ...
            do {
              ...
              if (
                cts._tag === "SwitchChainComplete" ||
                cts._tag === "WriteContractComplete"
              ) {
  ...
  const massagedDenom = $derived(
    pipe(
      step,
      Option.map((step) =>
        isHex(step.token) ? step.token : toHex(step.token),
      ),
    ),
  );

  $effect(() => {
    console.log({ step, sourceChain });
  });
</script>

<div class="grow relative min-w-full p-4 flex flex-col justify-between h-full">
  {#if Option.isSome(step) && Option.isSome(sourceChain) && Option.isSome(massagedDenom)}
    <div class="grow flex flex-col gap-4">
      <h3 class="text-lg font-semibold">
        Approve
        <TokenComponent chain={sourceChain.value} denom={massagedDenom.value} />
      </h3>
      <section>
        <Label>Current</Label>
        <TokenComponent
          chain={sourceChain.value}
          denom={massagedDenom.value}
