...
import { Array as Arr, Cause, Effect, Exit, Match, Option, pipe, Struct } from "effect"
...
const step = $derived(
  lts.pipe(Option.map(Struct.get("steps")), Option.flatMap(Arr.findFirst(is("ApprovalRequired"))))
)
...
const massagedDenom = $derived(
  pipe(
    step,
    Option.map(step => (isHex(step.token) ? step.token : toHex(step.token)))
  )
)
