  ...
  {#if connectStatus === "connected" && address?.length > 0}
    ...
    <div class="w-full bg-zinc-100 dark:bg-zinc-900 rounded-lg p-4 transition-colors">
      <div class="flex items-center justify-between">
        ...
        <button
          class="p-2 rounded-lg border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors"
          onclick={() => onDisconnectClick()}
          aria-label="Disconnect wallet"
        >
          <SharpPowerIcon class="size-4 text-zinc-400 hover:text-red-500 transition-colors" />
        </button>
      </div>
    </div>
  {/if}
