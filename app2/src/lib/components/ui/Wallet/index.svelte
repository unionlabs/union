...
>
  <section class="p-6 pb-4 border-b border-zinc-200 dark:border-zinc-800 flex-none">
    ...
    <nav class="flex gap-1 mt-4 relative" aria-label="Tabs">
      <div
        class="absolute bg-zinc-100 dark:bg-zinc-800 rounded-md transition-all duration-300"
        style:top="0"
        style:left={currentWalletType === 'all' ? '0' : currentWalletType === 'evm' ? '33.333%' : '66.666%'}
        style:width="33.333%"
        style:height="100%"
      />
      <button 
      ...
      >
        <div class="flex items-center justify-center gap-2">
          <span>EVM</span>
          <span class="w-1.5 h-1.5 rounded-full transition-all duration-300 ring-1 ring-opacity-20 {evmConnected 
      ...
      >
        <div class="flex items-center justify-center gap-2">
          <span>Cosmos</span>
          <span class="w-1.5 h-1.5 rounded-full transition-all duration-300 ring-1 ring-opacity-20 {cosmosConnected 
            ? 'bg-green-500 animate-pulse ring-green-500 shadow-[0_0_6px_0px_rgba(34,197,94,0.6)]' 
            : 'bg-white/10 dark:bg-white/5 backdrop-blur-sm ring-white/20'}">
          </span>
        </div>
