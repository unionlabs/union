<script lang="ts">
  ...
  function createParticleFromTransfer(transfer: TransferListItem) {
    ...
    if (Option.isSome(chains.data)) {
      const chainData = chains.data.value
      const sourceChain = chainData.find(c => c.universal_chain_id === transfer.source_chain.universal_chain_id)
      const destChain = chainData.find(c => c.universal_chain_id === transfer.destination_chain.universal_chain_id)
  ...
  function animate() {
    ...
    chainNodes.forEach((node, chainId) => {
      ...
      const outerGlow = ctx.createRadialGradient(
        node.x, node.y, 0,
        node.x, node.y, nodeRadius * (isSelected ? 2.2 : 1.5)
      )
...
<Card class="h-full">   
  <div class="relative w-full h-full" bind:this={containerElement}>
    {#if selectedFromChain && selectedToChain}
      <div class="absolute top-0 left-0 z-10 rounded-lg text-white min-w-48" style="background-color: {COLOR_CONFIG.uiBackground};">
