...
import {
  ...
  transferListPendingAckPageLtQuery,
  transferListPendingAckPageGtQuery,
  ...
  const initializeQuery = async () => {
    ...
    if (pageParam) {
      if (pageParam.startsWith("-")) {
        ...
        effect = transferListPendingAckPageGtQuery(sortOrder, settingsStore.pageLimit)
      } else {
        // Less-than query (next page)
        // @ts-ignore pageParam is not strictly a SortOrder, but this is desired behavior
        effect = transferListPendingAckPageLtQuery(pageParam, settingsStore.pageLimit)
      }
    } else {
      // No page param, load latest
      effect = transferListPendingAckQuery(settingsStore.pageLimit)
    }
