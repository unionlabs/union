...
<Sections>
  <Card divided>
    ...
    {:then bond}
      {@const overallStatus = (bond.bond_success === false || bond.delivery_success === false)
        ? "failure"
        : (bond.bond_success === true && bond.delivery_success === true)
        ? "success"
        : "pending"}
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              {#if bond.destination_chain}
                <ChainComponent
                  chain={bond.destination_chain}
                  withToken={bond.quote_token}
                />
              {:else}
