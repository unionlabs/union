...
const onLive = async () => {
  if (Option.isSome(transferList.data)) {
    await transferList.runEffect(
      transferListLatestQuery(settingsStore.pageLimit, settingsStore.mainnetOnly),
    )
...
const onPrevPage = async () => {
  if (Option.isSome(transferList.data)) {
    ...
    await transferList.runEffect(
      transferListPageGtQuery(firstSortOrder, settingsStore.pageLimit, settingsStore.mainnetOnly),
    )
...
const onNextPage = async () => {
  if (Option.isSome(transferList.data)) {
    ...
    await transferList.runEffect(
      transferListPageLtQuery(lastSortOrder, settingsStore.pageLimit, settingsStore.mainnetOnly),
    )
