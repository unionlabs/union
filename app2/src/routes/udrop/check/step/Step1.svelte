...
import { runPromise, runPromiseExit$ } from "$lib/runtime"
...
let shouldCreateEntry = $state(false)

const createEntryResult = runPromiseExit$(() =>
  shouldCreateEntry
    ? Effect.gen(function*() {
      const success = yield* Option.match(dashboard.airdrop, {
        onNone: () => Effect.succeed(false),
        onSome: (store) => Effect.tryPromise(() => store.createEntry()),
      })
...
const isCreatingEntry = $derived(shouldCreateEntry && Option.isNone(createEntryResult.current))

function handleCreateEntry() {
  if (shouldCreateEntry && Option.isNone(createEntryResult.current)) {
...
function handleLogin(provider: AuthProvider) {
  ...
  runPromise(
    Effect.gen(function*() {
      yield* dashboard.login(provider.id, "/udrop/check")
    }).pipe(
    ...
    <div class="flex flex-col gap-6 p-4 z-10 justify-between h-full">
      ...
      {#if Option.isSome(dashboard.session)}
        <div class="space-y-3">
          <Button
            variant="primary"
            class="flex w-full items-center justify-center gap-3 {isCreatingEntry ? 'opacity-70 cursor-not-allowed' : ''}"
