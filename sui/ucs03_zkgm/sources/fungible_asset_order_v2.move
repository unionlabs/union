...
module zkgm::fungible_asset_order_v2 {
    use zkgm::zkgm_ethabi;

    use std::string::{String};

    public struct FungibleAssetOrderV2 has copy, drop, store {
        sender: vector<u8>,
        receiver: vector<u8>,
    ...
    public fun sender(order: &FungibleAssetOrderV2): &vector<u8> {
        &order.sender
    }

    public fun receiver(order: &FungibleAssetOrderV2): &vector<u8> {
        &order.receiver
    }

    public fun base_token(order: &FungibleAssetOrderV2): &vector<u8> {
        &order.base_token
    }

    public fun base_amount(order: &FungibleAssetOrderV2): u256 {
        order.base_amount
    }

    public fun metadata_type(order: &FungibleAssetOrderV2): u8 {
        order.metadata_type
    }

    public fun metadata(order: &FungibleAssetOrderV2): &vector<u8> {
        &order.metadata
    }

    public fun quote_token(order: &FungibleAssetOrderV2): &vector<u8> {
        &order.quote_token
    }

    public fun quote_amount(order: &FungibleAssetOrderV2): u256 {
        order.quote_amount
    }

    public fun encode(order: &FungibleAssetOrderV2): vector<u8> {
    ...
    public fun decode(buf: &vector<u8>): FungibleAssetOrderV2 {
        let mut index = 0;
        FungibleAssetOrderV2 {
    ...
    fun test_encode_fungible_asset_order() {
        ...
        // let expected_packet = FungibleAssetOrderV2 {
        //     sender: b"AAAA",
        //     receiver: b"BBBB",
        //     base_token: b"CCCC",
        //     base_amount: 100,
